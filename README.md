...
联网对照 KONAMI 官方卡文与补充说明，复核 11 张参考样本，修正速攻魔法的发动前提、效果与费用、分支、自锁起点、灵摆区域与处理表示。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 11 张，未标注 14,705 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。

效果词表包含 19 个当前标签与 1 个兼容旧标签，保留已有标签 ID 和本机个人资料。规范见[卡片标注体系](docs/card-annotation-system.md)，后续智能体逐批标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)和只读校验器 `python scripts/check_card_annotations.py --runtime .local/YGOPro-Lite`。实现与验收范围见[1.46.0 验证记录](docs/verification-1.46.0.md)。
