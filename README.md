...
## 当前版本 · 1.49.8（续接英雄、No.与银河卡片）

继续核对原386张任务的第101～240项，新增113张有逐卡OCG依据的标注，27张保留明确模型、来源或裁定缺口；累计2,682／14,716张，尚余12,034张。原任务前240项共核准194张、46张待核实，后146项仍未处理，三个系列尚未全部完成。[逐卡清单](docs/card-annotation-batch-2026-09-26-fast-series-stage02.json)、[验收记录](docs/verification-1.49.8.md)和[全库进度](docs/card-annotation-progress.md)记录本阶段范围与续接位置。

处理数量可分别保存常量范围、发动时或处理时确定的动态依据；基本分减半与烧血／支付费用分开，攻击宣言、伤害计算及同名适用一次不混作每回合使用一次。对方暂时除外怪兽的返回保持非特殊召唤语义，来源纠错和异画卡号分别保留；旧2,569条规范记录逐项不变。因共用协议和展示变化，本阶段单独完整检查和打包验收。

### 上一版 · 1.49.7（统一批次首阶段与额外卡组确认）

从冻结的No.、英雄、银河386张剩余卡清单中，前100张进入本阶段，新增81张有逐卡OCG依据的标注，累计2,569／14,716张；19张因独立效果单元、来源／补足冲突或表达边界待核实。剩余286张继续保留原清单，不宣称三个系列已全部完成。本阶段引入共用模型变化，单独完成相关检查与当期打包验收，不把20张检查点当作整轮目标。[逐卡清单](docs/card-annotation-batch-2026-09-26-fast-series-stage01.json)、[验收记录](docs/verification-1.49.7.md)与[全库进度](docs/card-annotation-progress.md)分别记录范围和结果。

额外卡组确认与主卡组翻开分开检索；每战斗步骤、同一连锁、同名决斗中一次分别保存，动态伤害／回复只显示计算依据，不执行表达式。区分取除素材的费用与处理、固定获赋效果的独立费用与时点；纠正新批次原包的卡片身份错配并保留原始错误资料，旧2,488条正式记录逐项不变。

### 上一版 · 1.49.6（继续核对通常陷阱与玩家处理）

下一组100张通常陷阱完成逐卡处理，新增95张，累计2,488／14,716张；3张裁定证据不足、2张尚需独立效果模型的卡暂缓。准确区分迫使玩家送墓与直接效果送墓、伤害转回复与本卡直接回复、投币效果对调与重新投币、战斗计算与效果伤害，并支持处理时确定的动态基本分和明确的对方除外状态。费用、对象、固定后续效果、伤害步骤及部分处理分别保存。旧2,393条逐项保留，范围与结果见[逐卡清单](docs/card-annotation-batch-2026-09-26-traps4.json)、[本版验收](docs/verification-1.49.6.md)及[全库进度](docs/card-annotation-progress.md)。

### 上一版 · 1.49.5（继续补充效果怪兽与特殊攻击处理）

下一组100张效果怪兽完成逐卡处理，新增99张，累计2,393／14,716张；《再生僵尸》因官方卡文与补足的保护条件冲突暂缓。准确区分攻击回手手续与效果回手、战斗伤害视为效果伤害与固定数值烧血、诱发确认抽到的卡与永续公开手卡，并保留费用、取对象、强制／任意诱发及错过时点边界。[逐卡清单](docs/card-annotation-batch-2026-09-26-monsters2.json)、[本版验收](docs/verification-1.49.5.md)和[全库进度](docs/card-annotation-progress.md)记录来源与检查。旧2,294条逐项保留，尚余12,323张继续处理。

### 上一版 · 1.49.4（继续整合魔法陷阱与效果怪兽）

逐卡处理下一批400张，新增397张有完整OCG依据的标注：199张通常魔法、99张通常陷阱、99张效果怪兽，累计2,294／14,716张；3张规则依据不足或来源冲突的卡暂缓。补齐34张怪兽的通用TAG，支持按发动时卡文依据显示动态对象数量，并修正3张旧卡错误显示为固定1个对象的问题。仅自身的攻击限制和召唤成功响应窗口封锁仍能按限制TAG检索，但不会据此生成持续终场干扰候选。[逐卡清单](docs/card-annotation-batch-2026-09-26-short2.json)、[版本验收](docs/verification-1.49.4.md)与[全库进度](docs/card-annotation-progress.md)记录范围及检查；其余可核实卡片继续分批处理。

### 上一版 · 1.49.3（继续整合魔法陷阱与修正组合查询）

逐卡审核 400 张通常魔法／通常陷阱后，新增 395 张有完整 OCG 依据的标注，累计 1,897／14,716 张；5 张裁定资料不足的卡保留待核实。支持可变对象数量、阶段跳过、效果中发动场地魔法、随机结果及抽卡替代等规则。动作和来源／去向现在必须在同一个处理项中匹配，避免把“从卡组除外、以后从除外加手”误查为“从卡组加手”。同时修正了四张卡把“正在发动的这一张”误写成排除所有同名卡的候选结构。[逐卡清单](docs/card-annotation-batch-2026-09-26-short.json)、[版本验收](docs/verification-1.49.3.md)和[全库进度](docs/card-annotation-progress.md)记录本轮范围与检查；其余可核实卡片继续处理。

### 上一版 · 1.49.2（接收全库原文并分阶段整合）

对采集包的 14,981 个本地卡号建立接收审计，保留原始资料；候选来源不会自动变成已核对标注。本阶段新增 730 张通常／通常调整怪兽、84 张无效果的融合等特殊召唤怪兽，以及急袭猛禽 46 张、超重武者 47 张，原有 595 条不改写，累计 1,502／14,716 张非衍生物。素材手续、费用、伤害防止、守备攻击与固定获赋效果分别记录；固定获赋效果的时点、费用和次数可独立检索并显示依据，不与授予层拼接条件。资料缺口及后续范围见[接收审计](docs/card-source-intake-2026-09-26.md)和[全库进度](docs/card-annotation-progress.md)，本阶段检查见[验收记录](docs/verification-1.49.2.md)。其余卡片持续分批核对，未标注不代表没有能力。

### 上一版 · 1.49.1（六个完整系列继续补充效果标注）

在现有统一标注与跨模块能力读取基础上，按本地整数卡号新增 97 张已核对卡牌，补齐卫星闪灵、铁兽、龙辉巧、随风旅鸟、十二兽、朋克六个完整系列。逐卡保存完整卡文、全部分段与 KONAMI 官方 OCG 卡页／FAQ 来源，费用、非入连锁召唤、仪式及超量素材、追加通常召唤和回合自锁分别记录。[逐卡清单](docs/card-annotation-batch-2026-09-25-phase5.csv)列出卡号、卡名、系列、来源和核对结果；[验收记录](docs/verification-1.49.1.md)列出实际检查。该阶段完成 595／14,716 张非衍生物，尚余 14,121 张；后续进度见[全库账本](docs/card-annotation-progress.md)。自动草稿不计入已核对。

### 上一版 · 1.49.0（标注直接驱动用途库与起手分析）

终场、手坑、解场资料由统一标注自动生成，直接替换同一卡牌原先单独维护的用途、条件和效果备注；旧个人内容写入前完整备份。起手的角色与统计也改为读取统一效果，手坑／解场 TAG 自动同步成员。界面以规范效果为主体，通过“编辑统一标注”集中维护，不再并排维护两套说明。1.49.0 验收时按当时 498 张标注生成 91 张终场、35 张手坑、174 张解场用途记录，其中“标注推导”明确显示为静态分类依据，具体费用、时点及可用次数仍须核对。使用与恢复见[跨模块能力说明](docs/card-capability-integration.md)，升级与实测见[1.49.0 验收](docs/verification-1.49.0.md)。
...
联网对照 KONAMI 官方卡文与补充说明，复核首批 11 张参考样本后补全引擎审计清单 35 张、手坑／解场常用卡 62 张；随后按完整系列和实战辅助缺口推进，并从全库采集原文中逐卡核对整合。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 2,682 张，未标注 12,034 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。

效果词表包含 20 个当前标签与 1 个兼容旧标签，伤害防止与造成效果伤害分开；受控处理、次数与获赋效果的定义及正反例见[卡片标注体系](docs/card-annotation-system.md)。规则段、处理说明和来源按需展开，点击「个人修正」才显示编辑。逐批效果标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)、实际 runtime 校验器，以及[无效果专项检查](scripts/check_annotation_no_effect_batch.py)和[两系列语义检查](scripts/check_annotation_series_batch_20260926.py)。

开发流程新增[统一批次整理与续接工具](docs/card-annotation-pipeline.md)：按完整系列冻结清单、并行准备忠实来源正文、用结构化决策代替重复批次脚本，保存审核／待核实／合并断点。检查缓存绑定实际卡文、来源内容、词表与代码，合并前强制完整检查并备份。每约20张检查，机制稳定的约300～500张完整发布阶段统一验收；工具不自动核准规则，工具不自动计入覆盖，当前正式覆盖为2,682张。[本轮工具验证](docs/verification-annotation-pipeline-2026-09-26.md)与卡片新增数量分开记录。
...
| 让其他 AI 采集打包卡片资料，再由 Codex 整理与整合 | [采集交接教程](docs/card-annotation-external-ai-tutorial.md)、[采集提示词](docs/card-source-collector-prompt.md) |
