...
> **BO1 当前为手动先攻辅助。** BO3、自动识别、自动决定先后攻和后攻突破尚未实现。详见 [BO1 决斗说明](docs/duel.md)。
...
- **紧凑卡组盒子**：三个代表卡展示位，卡组管理、展开选卡与决斗选卡共用展示方式。
- **起手与卡组排布**：起手槽居中，候选卡组收紧并按类型、等级和卡号排序。
- **图文展开教程**：鼠标点选和快捷键自动聚焦当前步骤；长详情在流程图外滚动阅读，按下鼠标会暂停悬停，前进／后退按钮保持可点。
- **方案预览**：卡片显示主线步数与可用分支数；简略保留完整终场备注，详细显示分支时点、条件、实际干扰与终场。
- **统一教程快捷键**：所有入口、软件内外共用同一套配置；快捷键窗口独立，起手张数保留在全局设置。
- **入口与弹窗**：首页提供卡组编辑、展开、决斗和 TAG 管理四个快速入口；一图流弹窗避开系统窗口按钮。

完整更新、操作说明与验收边界保留在[使用与开发手册](docs/usage-guide.md)。
