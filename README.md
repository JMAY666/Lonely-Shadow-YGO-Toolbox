...
决斗依次经过「模式选择 → 功能选择 → 卡组选择 → 决定先/后攻 → 卡组展开」。功能选择下提供「手动或自动 / 平台选择」：手动进入现有卡组流程；自动进入 YGOPro（初代原版）平台页，再经过「卡组识别 → 卡牌预览」。自动模式当前为前端演示，支持示例缩略图、卡牌详情与标记、名称和主副 TAG 调整、会话内保存及同名覆盖确认；不会识别真实游戏、写入卡组库或启动自动决斗。
...
- **自动选择前端**：完成 YGOPro 平台选择、左右分栏的卡组识别、复用手动能力的卡牌预览，以及示例 AI TAG、手动调整、保存和同名覆盖更新交互。右下角保留「开始决斗」，目前只提示后续接入状态。真实功能留待下一阶段实现。
...
1.20.0 新增自动选择的前端流程，操作边界见 [BO1 决斗说明](docs/duel.md)，检查结果见 [1.20.0 验收记录](docs/verification-1.20.0.md)。保留 1.19.1 的原生游戏画面绘制隔离，详情见[1.19.1 验收记录](docs/verification-1.19.1.md)。
