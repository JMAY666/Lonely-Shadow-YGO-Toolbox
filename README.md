...
> **BO1 先攻可根据手动录入或自动捕捉的起手筛选方案、生成本局临时路线，并按步骤图确认操作。** 后攻已开放独立局面记录：冻结原始起手，人工填报当前资源、实际费用、抽牌、公开卡牌及关键窗口。交康推荐、后攻路线和完整自动跟踪仍在分期实施；BO3 尚未实现。详见 [BO1 决斗说明](docs/duel.md)与[后攻执行计划](docs/bo1-second-execution.md)。
...
本次开放 BO1 后攻局面记录，为[执行计划](docs/bo1-second-execution.md)的 P1.1。手动选择后手，或在已适配平台完成后攻开局识别后，可记录当前手牌、费用、实际抽牌、双方已知区域、场上位置、素材、次数与限制备注，以及当时的响应窗口和实际选择。历史保存在本机，可从决斗入口回看；人工记录恢复时重新核对，旧窗口不会恢复。当前不是规则状态重建或后攻推荐，不能据此声称效果可合法发动或能够斩杀。实际检查见 [1.29.0 验证记录](docs/verification-1.29.0.md)。
