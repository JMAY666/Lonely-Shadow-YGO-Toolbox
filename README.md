...
本次将对方手牌展示改为等量随机卡背，并让一图流的复杂 Step 加宽排版、短 Step 贴合内容；规则快照补充普通召唤上限与额外召唤使用状态。范围与验证见 [1.28.3 验证记录](docs/verification-1.28.3.md)，持续适用效果与离场后的限制见[规则状态说明](docs/continuous-rules.md)。
...
- **自动模式方案与教程**：确认先攻起手后，根据本局卡组、TAG 与冻结起手筛选方案；支持排序、收藏、详细预览、单步自适应展示、分支导航、场面查看、快捷键及临时方案生成和续算。手动与自动教程一次只展示当前 Step，长内容滚动，不被后续大步骤缩小；展示卡牌有卡图与名称，旧方案按冻结事件补显示。自动模式拥有独立页面、状态、接口和推演会话，与手动模式共用规则和数据服务，筛选与教程进度互不覆盖。见 [BO1 决斗说明](docs/duel.md)。
