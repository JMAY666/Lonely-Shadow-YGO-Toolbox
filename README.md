...
### 当前版本 · 1.42.0 · 卡组知识包与制作工作区

左侧「知识包」提供已安装内容与独立制作入口。构筑、展开、妥协、终场和对策按主题关联，保留原始资料、稳定 ID、修订与依赖；可持续编辑、发布完整文件、安装启停、处理个人修改冲突并回退版本。启用内容分别出现在构筑、展开管理和情报站，重复安装不会重复导入。

具备原始决策证据的内容可按明确起手、场景和预算进行本机规则验证，支持取消、按场景恢复和有效结果复用；得到单独计算许可后接入现有匹配及模块化核心。文字资料不会自动变成操作，来源的验证声明不直接授予本机执行资格。内置烙印资料样例保持未逐条引擎验证状态。

使用、格式、兼容与验证范围见[知识包说明](docs/knowledge-packages.md)及[1.42.0 验收记录](docs/verification-1.42.0.md)。本功能独立于已封存的 AI 学习计划，没有训练模型或恢复旧实验。
