...
联网对照 KONAMI 官方卡文与补充说明，复核首批 11 张参考样本后补全引擎审计清单 35 张、手坑／解场常用卡 62 张；随后按完整系列和实战辅助缺口推进，并从全库采集原文中逐卡核对整合。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 2,488 张，未标注 12,228 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。
...
开发流程新增[统一批次整理与续接工具](docs/card-annotation-pipeline.md)：按完整系列冻结清单、并行准备忠实来源正文、用结构化决策代替重复批次脚本，保存审核／待核实／合并断点。检查缓存绑定实际卡文、来源内容、词表与代码，合并前强制完整检查并备份。每约20张检查，机制稳定的约300～500张完整发布阶段统一验收；工具不自动核准规则，正式覆盖仍为2,488张。[本轮工具验证](docs/verification-annotation-pipeline-2026-09-26.md)与卡片新增数量分开记录。
