...
## 当前版本 · 1.46.0（入口归类与卡片标注完善）

主导航归为「首页／构筑训练／决斗／资料库」：卡组编辑、展开与模块化在构筑训练顶部切换；卡片标注、情报站与 TAG 管理在资料库顶部切换。全局设置独立保留在侧栏底部。已有卡组编辑缓冲、训练进度与个人资料继续保留。

卡片标注默认用于查阅：顶部搜索，左侧紧凑卡片列表，右侧阅读当前卡片。高级筛选按资源获取、召唤与展开、除去与转移、无效与干扰、保护与耐性、行动限制、信息与数值、规则与手续分组；规则段、完整处理说明和来源信息按需展开，点击「个人修正」才显示标签与备注编辑。可以浏览全部卡片、未标注卡及无效果卡，能力条件仍只匹配已有标注。
...
效果词表包含 19 个当前标签与 1 个兼容旧标签，受控词表按第二批需求新增不产生连锁块效果、魔法效果发动等效果类别与盖放、超量素材转移、除外／送墓／超量素材取除费用等取值，保留已有标签 ID 和本机个人资料。规范见[卡片标注体系](docs/card-annotation-system.md)，后续智能体逐批标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)和只读校验器 `python scripts/check_card_annotations.py --runtime .local/YGOPro-Lite`。实现与验收范围见[1.46.0 验证记录](docs/verification-1.46.0.md)。
