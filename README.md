...
情报站沿用卡组编辑和展开管理的工作区布局：顶部切换终场、手坑与断点分类，左侧集中搜索、筛选和资料列表，右侧编辑当前资料。进入分类时打开首条资料，当前选择在列表中高亮；备注可独立滚动，保存操作保留在编辑区底部。选卡和来源汇总使用完整工作区，窄窗口改为上下排列。

左侧「全局设置」上方的「情报站」集中管理通用终场标注、手坑资料和手动断点。已有方案标注可以按卡牌合并，也可以一键全部合并：同一卡牌只保留一份通用资料，效果取并集，相同备注去重，不同的卡牌用途与效果备注并列展示、独立编辑，并保留来源。效果文本不一致时保留为待核对，不按效果序号强行对应。
...
全部资料沿用本地 TAG 文件的原子保存、版本冲突检查和备份，卡库缺卡时保留编号和用户资料。使用方式、兼容与恢复见[情报站说明](docs/intelligence.md)，布局验证见[1.38.2 验证记录](docs/verification-1.38.2.md)，标注合并见[1.38.1 验证记录](docs/verification-1.38.1.md)，首版范围见[1.38.0 验证记录](docs/verification-1.38.0.md)。
...
| 学生 GPU 训练、动态状态契约、CPU 连续执行与引擎重放（开发实验，未接入正式界面） | [学生训练与执行试验](experiments/ygo_learning/README.md) |
