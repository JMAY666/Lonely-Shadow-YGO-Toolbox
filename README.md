...
> **BO1 先攻可根据手动录入或自动捕捉的起手筛选方案、生成本局临时路线，并按步骤图确认操作。** 后攻支持独立局面记录及有限本地案例的[条件化交康提示](docs/bo1-second-hints.md)，原始起手与当前资源分开保存。外部平台规则未核实的提示保持信息不足；有完整日志的内置练习可进行[后攻路线续算](docs/bo1-second-routes.md)；外部状态重建、完整自动跟踪和 BO3 仍待实施。详见 [BO1 决斗说明](docs/duel.md)与[后攻执行计划](docs/bo1-second-execution.md)。
...
YGOPro 公开资源核对增加[公开连锁条目与有限我方选择](docs/bo1-second-chain.md)：显示读取当时的来源、处理开始标记和客户端列出的选项；采用前再次核对，选项关闭或连锁变化也会使旧快照失效。现场已核对手坑选项、发动后的资源变化及一个连锁 1 窗口。客户端选项不等于规则认证或交康推荐，完整事件、对象、费用、无效类型和持续限制仍待接入。实际检查范围见[1.36.0 验证记录](docs/verification-1.36.0.md)。

### 此前版本 · 1.35.0

YGOPro 公开资源核对新增[可见阶段、已观察的回合玩家与素材归属](docs/bo1-second-context.md)。只在连续读取中捕捉到对应回合切换时填写回合玩家，错过或中断则保持待核对；素材与承载怪兽同时核对，避免跨区域重复计数。用户已实机确认主要阶段 1 → 战斗 → 主要阶段 2 及两张素材的归属。完整事件、连锁、合法响应窗口和规则重建仍未接入，检查范围见[1.35.0 验证记录](docs/verification-1.35.0.md)。

### 此前版本 · 1.34.0

已适配 YGOPro 的自动后攻入口增加[公开资源只读核对](docs/bo1-second-live.md)：先预览，再重新读取并确认采用当前手牌、可见区域、区域数量及 LP；保存原始起手与旧人工历史。局面变化、读取中断或换局会停止沿用旧结果。回合玩家、阶段、连锁、素材和规则次数仍需核对，外部场面不能直接用于内置路线或战斗验证。本次同时修复 YGOPro 默认模块名导致的定位失败。实际客户端与隔离测试分别见[1.34.0 验证记录](docs/verification-1.34.0.md)。

### 此前版本 · 1.33.0

内置后攻工作区增加[指定攻击顺序的条件化验证](docs/bo1-second-battle.md)：同步主要阶段 1 或战斗操作点后，按当前怪兽与公开目标选择攻击顺序，由隔离规则核心检查攻击资格、目标、次数及伤害。演练与真实资源分开保存，实际攻击后重新同步，旧结果只供回看。已覆盖攻击限制、守备目标、次数耗尽和未知目标等反例，并补齐假面变换的三种处理案例；这不是全面斩杀搜索，也不认证外部 MD 规则。检查结果见[1.33.0 验证记录](docs/verification-1.33.0.md)。

### 此前版本 · 1.32.0

内置后攻练习现可从对手首回合响应窗口接入，在同一记录内保留交康提示、人工条件和计划，随后同步实际费用、抽牌及我方回合局面继续续算。关联后禁止重复手填资源变化；窗口复盘分开展示当时已知信息、玩家备注和随后真实结果。详见[两阶段操作说明](docs/bo1-second-flow.md)与[1.32.0 验证记录](docs/verification-1.32.0.md)。外部完整状态、复杂响应策略和战斗斩杀仍未开放。

### 此前版本 · 1.31.0

本次完成有限内置练习的[后攻规则重建与路线续算](docs/bo1-second-routes.md)：在独立引擎中恢复同一练习的完整历史，核对实际抽牌、费用、次数和对手场面，再按已有来源比较解场与展开。实际操作偏离或受到干扰后，同步最新已结算节点，保留已执行历史和原始起手，重新计算未执行后续。候选假设对手不追加响应；选择计划不向游戏输入。外部平台完整状态及战斗斩杀仍未验证，检查范围见 [1.31.0 验证记录](docs/verification-1.31.0.md)。
...
此前的原始起手、当前资源、实际费用／抽牌、公开区域和关键窗口记录继续保留，见 [1.29.0 验证记录](docs/verification-1.29.0.md)。外部对局规则状态重建、完整后攻路线和斩杀判断仍未接入。
