...
## 当前版本 · 1.45.0（卡片效果结构化标注首期）

新增「卡片标注」模块：以效果为原子单位建立卡片能力知识——卡文自动分段（①②编号、前置规则文本、灵摆双区），逐效果标注发动条件、费用、对象、处理内容（动作、数量、来源／去向区域）、次数与持续限制，并用 18 个通用效果 TAG（加入手卡、特殊召唤、无效、除外、破坏、封锁等，含定义与同义词）表达共性能力。组合查询默认要求所有条件在**同一效果**内成立并逐条给出命中依据；同一 TAG 的卡片仍按来源区域等参数区分（如「E-紧急呼唤」仅卡组检索、「暗黑爆发」仅墓地回收），互不串用。覆盖清单按 未标注／自动草稿／部分／已核对／已确认／待核对／卡文已变化 七态逐卡展示——未标注不代表没有该能力，全卡库 14,716 张可标注卡当前仅完成 11 张代表性样本（多效果、分支、互斥限制、灵摆、无效果卡；其中 3 张带引擎验证映射）。

标注区分自动草稿、人工核对与引擎验证；卡库更新后指纹失配的标注冻结为待核对并保留个人备注、修正与备份。TAG 不参与卡组／方案的系列自动识别，查询时可与现有系列／用途 TAG 组合。静态标注不判断当前局面能否发动，运行时判断仍由规则引擎负责。规范见[卡片标注体系](docs/card-annotation-system.md)，现状缺口见[分析文档](docs/card-annotation-gap-analysis.md)，测试与耗时见[1.45.0 验证记录](docs/verification-1.45.0.md)。
