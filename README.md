...
## 当前版本 · 1.47.0（系列卡牌夹与彩色标注）

主导航归为「首页／构筑训练／决斗／资料库」：卡组编辑、展开与模块化在构筑训练顶部切换；卡片标注、情报站与 TAG 管理在资料库顶部切换。全局设置独立保留在侧栏底部。已有卡组编辑缓冲、训练进度与个人资料继续保留。

卡片标注默认显示按中文名称排列的系列卡牌夹，封面使用本地代表卡图；多系列归属保留，「无系列归属」单独收纳。进入卡牌夹后查看卡片列表与效果，也可以切换「全部卡片」。搜索支持中文系列名、旧译名、卡名与卡号。筛选整体默认收起，当前已选条件显示计数；原卡图默认收起，点击「查看原卡图」加载。效果 TAG 按八类语义配色，融合／同调／超量／连接怪兽显示文字和专用符号，灵摆与调整可叠加标记。

本轮为青眼、转生炎兽、相剑、驱魔姐妹、杀手旋律制作五套系列徽记，联网核对七张代表卡。系列名采用已核对的官方简体中文，仍可搜索「救祓少女」「杀手级调整曲」等旧译名；卡库原卡名、卡文、个人 TAG 与备注保留。其他系列从本地卡库生成普通卡牌夹和封面，尚未逐系列联网核对；徽记为项目设计的识别图形。可直接交给其他智能体继续扩充的[系列标识制作流程](docs/card-series-production-guide.md)包含来源、命名、封面、样例和校验步骤。

联网对照 KONAMI 官方卡文与补充说明，复核 11 张参考样本，修正速攻魔法的发动前提、效果与费用、分支、自锁起点、灵摆区域与处理表示；此后按引擎审计清单以官方 FAQ 补充说明的效果类别裁定补全 35 张（转生炎兽、相剑、杀手级调整曲等常用构筑）。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 46 张，未标注 14,670 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。

效果词表包含 19 个当前标签与 1 个兼容旧标签，受控词表按第二批需求新增不产生连锁块效果、魔法效果发动等效果类别与盖放、超量素材转移、除外／送墓／超量素材取除费用等取值，保留已有标签 ID 和本机个人资料。规则段、处理说明和来源按需展开，点击「个人修正」才显示编辑。规范见[卡片标注体系](docs/card-annotation-system.md)，逐批效果标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)和只读校验器 `python scripts/check_card_annotations.py --runtime .local/YGOPro-Lite`。本轮验收见[1.47.0 验证记录](docs/verification-1.47.0.md)。
