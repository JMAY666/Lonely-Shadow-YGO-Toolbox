...
决斗依次经过「模式选择 → 功能选择 → 卡组选择 → 决定先/后攻 → 卡组展开」。功能选择下提供「手动或自动 / 平台选择」：手动进入现有卡组流程；自动可选择 YGOPro 或 YGOPRO2，弹窗显示进程名称、PID、路径及重试入口。两个平台均可启动智能化识别；YGOPro 还可读取正在编辑的主／额外／副卡组，进入卡牌预览、设置 TAG 并保存到本地卡组库，同名覆盖需确认并保留旧版本备份。YGOPRO2 暂不开放编辑器卡组识别。
