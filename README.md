...
> **BO1 可根据已录入起手，在后台生成本局临时方案；按步骤图确认操作，填报随机结果或阻抗后重新计算后续。** 自动模式已支持识别我方先后攻并确认；BO3、自动起手及完整对局状态识别、后攻展开尚未实现。详见 [BO1 决斗说明](docs/duel.md)。
...
- **YGOPro 实时卡组捕捉**：以只读权限读取当前编辑器，包括尚未保存的修改；每次「获取卡组」重新读取，失败时禁止将上次快照作为新结果保存。支持本地 TAG 推荐、手动调整、持久化保存及有版本校验的同名覆盖。当前仅适配经指纹核实的 YGOPro 1.036.2 x64 构建，其他构建明确提示未适配；自动起手与展开尚未接入。适配范围见 [BO1 决斗说明](docs/duel.md)。
...
后续对局数据读取的已验证路径与边界见 [YGOPro 对局读取研究记录](docs/ygopro-live-state-research.md)；手牌等完整对局数据读取仍为研究能力。
...
1.22.0 新增自动先后攻识别与人工确认，操作边界见 [BO1 决斗说明](docs/duel.md)，检查结果见 [1.22.0 验收记录](docs/verification-1.22.0.md)。卡组捕捉的既有验证见 [1.21.0 验收记录](docs/verification-1.21.0.md)。
