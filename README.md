...
## 当前版本 · 1.47.5（继续按完整系列补充效果标注）

继续补齐烙印、死狱乡、自奏圣乐与青眼四个完整系列，新增 94 张本地整数卡号条目，复核并保留已有 6 张；四系列在当前卡库中分别覆盖 25／25、11／11、19／19、46／46。新增条目逐卡对照 KONAMI 官方 OCG 卡片页及 FAQ，青眼究极龙与青眼白龙的无效果异画卡号逐号保留身份。本轮实际卡号、卡名、系列、来源和跳过原因见[清单](docs/card-annotation-batch-2026-09-25-phase2.csv)，验收范围见[1.47.5 验证记录](docs/verification-1.47.5.md)。当前可标注的 14,716 张非衍生物中，内置完成 302 张，仍有 14,414 张未标注；后续继续按系列分阶段推进，进度见[全库账本](docs/card-annotation-progress.md)，不把自动草稿计入已核对。

### 上一版 · 1.47.4（按完整系列补充卡片效果标注）
...
联网对照 KONAMI 官方卡文与补充说明，复核首批 11 张参考样本后补全引擎审计清单 35 张、手坑／解场常用卡 62 张；随后按完整系列和实战辅助缺口补充 100 张，本轮继续补充 94 张。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 302 张，未标注 14,414 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。

效果词表仍包含 19 个当前标签与 1 个兼容旧标签；本轮只扩充确需使用的受控区域、处理、费用及时点，定义与正反例见[卡片标注体系](docs/card-annotation-system.md)。规则段、处理说明和来源按需展开，点击「个人修正」才显示编辑。逐批效果标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)、实际 runtime 校验器与[本轮语义检查](scripts/check_annotation_batch4.py)。
