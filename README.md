...
左侧「全局设置」上方新增「情报站」，集中管理通用终场标注、手坑资料和手动断点。终场卡牌浮层可查看最新通用标注、一键应用到当前实例，或明确选择同时保存为通用标注；已有方案及分支的实例、区域和局部备注保持独立。旧方案标记按卡牌汇总，逐个来源选择导入，不自动合并有差异的用途或效果。
...
全部资料沿用本地 TAG 文件的原子保存、版本冲突检查和备份，卡库缺卡时保留编号和用户资料。使用方式、兼容与恢复见[情报站说明](docs/intelligence.md)，实际验证范围见[1.38.0 验证记录](docs/verification-1.38.0.md)。
