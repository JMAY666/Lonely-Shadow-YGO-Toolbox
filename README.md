...
## 当前版本 · 1.47.3（卡片标注纠错与个人修正版本隔离）

修正《次元吸引者》的送墓费用说明、《三战之才》等 7 张卡的处理 TAG、35 项快速效果标志，以及《访问码语者》②的合法场上区域。加入效果类别／快速标志与直接处理／TAG 的一致性检查；场上区域筛选能包含明确登记的子区域，不从笼统的「场上」反推某个子区域一定可用。覆盖仍为 108 张，本轮没有新增卡牌。

个人修正按卡文指纹保存。卡文变化后，旧标签、备注和确认状态不会套用到新效果；未记录卡文版本的旧资料先作为待核对内容保留，可在「历史个人修正」查看。首次保存新版资料前自动备份原文件，旧资料继续保留，保存失败不改变当前内存和原文件。下一批 12 张卡的独立交接提示词见[补卡指示](docs/card-annotation-next-batch.md)，修复验收见[1.47.3 验证记录](docs/verification-1.47.3.md)。
...
联网对照 KONAMI 官方卡文与补充说明，复核 11 张参考样本，修正速攻魔法的发动前提、效果与费用、分支、自锁起点、灵摆区域与处理表示；此后按引擎审计清单以官方 FAQ 补充说明的效果类别裁定补全 35 张（转生炎兽、相剑、杀手级调整曲等常用构筑），再按缺口分析补全手坑／解场常用卡 62 张（灰流丽、无限泡影之外的 staples 全部，含增殖的G、深渊之兽、坏兽、禁忌系列、颉颃胜负与各类取控制／盖放解场）。卡文变化后显示保存的旧卡文，停止用旧标注匹配当前能力；自动草稿不能一键确认为完整标注。覆盖统计排除衍生物：当前基础库 14,981 张，可标注 14,716 张，已标注 108 张，未标注 14,608 张。未标注不代表没有能力，脚本映射也不等于完整规则验证。

效果词表包含 19 个当前标签与 1 个兼容旧标签，受控词表按第二、三批需求新增不产生连锁块效果、魔法效果发动等效果类别，盖放、超量素材转移、表示形式变更、夺取控制权等处理，除外／送墓／超量素材取除／指示物取除等费用取值，保留已有标签 ID 和本机个人资料。规则段、处理说明和来源按需展开，点击「个人修正」才显示编辑。规范见[卡片标注体系](docs/card-annotation-system.md)，逐批效果标注请使用[标注交接指南](docs/card-annotation-agent-guide.md)和只读校验器 `python scripts/check_card_annotations.py --runtime .local/YGOPro-Lite`。本轮验收见[1.47.1 验证记录](docs/verification-1.47.1.md)。
