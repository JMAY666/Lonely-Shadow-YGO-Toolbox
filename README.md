...
> **BO1 可根据已录入起手，在后台生成本局临时方案；按步骤图确认操作，填报随机结果或阻抗后重新计算后续。** 自动模式已支持先后攻识别、初始起手捕捉与确认；自动方案匹配、BO3、完整对局状态跟踪及后攻展开尚未实现。详见 [BO1 决斗说明](docs/duel.md)。
...
1.23.1 修复临时方案生成与续接的起点混用及连接素材的脚本兼容问题，检查结果见 [1.23.1 修复记录](docs/verification-1.23.1.md)。自动起手预览与确认的操作边界见 [BO1 决斗说明](docs/duel.md)，此前检查见 [1.23.0](docs/verification-1.23.0.md)、[1.22.0](docs/verification-1.22.0.md) 和 [1.21.0](docs/verification-1.21.0.md)。
