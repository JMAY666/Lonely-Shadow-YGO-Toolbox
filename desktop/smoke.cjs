...
(async () => {
  ...
  if(onlyIntelligence){
    if(process.argv.includes('--live-only')){
      ...
      await close();await launch();
      const restored=await page.evaluate(id=>api('/api/duel/live',{action:'read',id}),id);
      assert.deepEqual(restored.session.frozen,state.frozen);assert.deepEqual(restored.session.events,state.events);
      assert.equal(restored.session.needs_sync,true);assert.equal(restored.session.state.window,null);
      pass('Real restart retains private observed history and opening while refusing to restore an active response window');
      await close();assert.deepEqual(errors,[]);fs.writeFileSync(path.join(evidence,'live-result.json'),JSON.stringify({checks,errors,globalInput:false},null,2));return;
    }
    ...
    if(process.argv.includes('--matchups-only')){
      ...
      pass('Illustrated matchup records, sources and personal edits survive a full application restart');
      await close();assert.deepEqual(errors,[]);fs.writeFileSync(path.join(evidence,'matchups-result.json'),JSON.stringify({checks,errors,globalInput:false},null,2));return;
    }
    ...
    await require('./intelligence-smoke.cjs')({page,application,root,evidence,pass});
    const before=await page.evaluate(()=>api('/api/intelligence'));
    await close();await launch();
    assert.deepEqual(await page.evaluate(()=>api('/api/intelligence')),before);
    pass('Intelligence cards, folders, topics, ordered responses and TAG membership survive a full application restart');
  ...
  if(onlyAnnotations){
    await require('./card-annotations-smoke.cjs')({page,application,root,evidence,pass});
    await close();assert.deepEqual(errors,[]);fs.writeFileSync(path.join(evidence,'annotations-result.json'),JSON.stringify({checks,errors,globalInput:false},null,2));return;
  }
