'use strict';
const defaults = {back:'Control+Shift+Left',forward:'Control+Shift+Right',up:'Control+Shift+Up',down:'Control+Shift+Down',end:''};
...
function createController({registry,send,isFocused}) {
  let state = {active:false,enabled:false,suspended:false,session:'',bindings:defaults}, registered = [], error = '';
  function release() {for (const key of registered) registry.unregister(key); registered = [];}
  function invoke(action) {
    if (state.active && state.enabled && !state.suspended && !isFocused() && registered.includes(state.bindings[action])) send({action,session:state.session});
  }
  function sync() {
    release(); error = '';
  ...
  return {update,sync,status,stop,invoke};
}
module.exports = {defaults,normalize,createController};
