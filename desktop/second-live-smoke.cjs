...
module.exports=async({page,application,evidence,pass})=>{
 ...
 const handle=async route=>{
  ...
  if(action==='live-preview')doc.live_panel.preview={id:'preview',revision:doc.revision,created_ms:Date.now(),snapshot,current:true,can_apply:true};
  if(action==='live-apply'){
   assert(body.confirmed);assert.equal(body.preview_id,'preview');
   const before=structuredClone(doc.current);doc.revision++;
   doc.current={...doc.current,cards:structuredClone(cards),lp:[6000,8000],phase:'unknown',turn:2,turn_player:null};
   doc.live_link={snapshot_id:'preview'};doc.live_history=[{confirmed_ms:Date.now(),snapshot}];doc.live_panel={supported:true,history_count:1};
   ...
   doc.status_reason='阶段、素材与规则仍待核对';
  }
 ...
 try{
  ...
  assert.equal(await page.locator('#second-hand .second-card').count(),4);
  pass('Synthetic public-resource UI previews before applying, preserves original hand/history, masks hidden cards, blocks duplicate manual moves and labels unsynced state');
 }finally{
