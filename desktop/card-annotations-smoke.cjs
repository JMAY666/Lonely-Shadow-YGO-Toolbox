...
module.exports = async ({page, application, root, evidence, pass}) => {
  ...
  await page.locator('[data-anno-folder-page="24"]').click();
  assert.equal(await page.evaluate(()=>annoUI.folderOffset),24);
  ...
  assert.equal(await page.locator('#anno-art').getAttribute('open'),null);
  assert.equal(await page.locator('#anno-art img').getAttribute('src'),null,'full original art is not loaded until requested');
  await page.locator('#anno-art summary').click();
  await page.waitForFunction(()=>document.querySelector('#anno-art img').naturalWidth>0);
  await page.locator('#anno-art summary').click();
  assert.equal(await page.locator('#anno-art img').isVisible(),false);
