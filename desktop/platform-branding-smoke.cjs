...
module.exports=async({page,application,evidence,pass})=>{
  ...
  for(const width of [900,1280,1600]){
    ...
    for(const [index,slug] of ['ygopro','ygopro2','mdpro3'].entries()){
    ...
    if(width>=1280)assert(frames.every(frame=>Math.abs(frame.y-frames[0].y)<2),'Three platforms fit on one row at desktop widths');
