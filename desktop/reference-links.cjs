...
function openReference(shell,value){
  ...
  if(!['http:','https:'].includes(url.protocol)||url.username||url.password||!['db.yugioh-card.com','roadoftheking.com','masterduelmeta.com'].includes(host))throw new Error('参考资料链接不在已核对来源中');
