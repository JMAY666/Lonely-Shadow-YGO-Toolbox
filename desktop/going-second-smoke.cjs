...
module.exports=async({page,application,root,evidence,pass})=>{
  ...
  await page.locator('.opening-route').filter({hasText:plan.name}).locator('summary').click();
  assert.match(await page.locator('.opening-route').filter({hasText:plan.name}).innerText(),/终场来源效果/);
  ...
  await page.locator('.opening-card [data-review-card]').first().click();await page.waitForFunction(()=>!document.querySelector('#review-card-popover').hidden);
  ...
  await page.locator('[data-opening-scope] details').first().locator('summary').click();
