...
function updateModuleChrome() {
  ...
  for (const name of ['home','decks','expansion','tags']) {
...
async function switchModule(target) {
  if (!['home','decks','expansion','tags'].includes(target) || target===moduleUI.current) return;
  if (moduleUI.switching || app.busy || flow.busy || flow.saving || flow.confirming || rewindState.busy || tagManagerUI.busy || document.querySelector('dialog[open]')) {
  ...
  const destination = ['home','tags'].includes(target) ? moduleUI.editorOwner : target;
