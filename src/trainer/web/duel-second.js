...
function secondHistoryView(doc){
  return `<details class="second-panel"><summary>实际观察历史 · ${doc.events.length} 项</summary><p>这里记录的是当时的人工确认与观察，不是自动识别或推演结论。</p><ol class="second-journal">${doc.events.map(e=>`<li><details><summary>${escape(e.summary)}</summary><p>${new Date(e.time_ms).toLocaleString()} · 人工确认 · 版本 ${e.revision}</p><p>当时阶段：${e.before.turn_player?'对手':'我方'}第 ${e.before.turn} 回合 · ${escape(SecondDuelModel.phases[e.before.phase])}</p><p>当时我方手牌：${e.before.cards.filter(c=>c.controller===0&&c.location===2).map(c=>escape(secondName(doc,c.code))).join('、')||'无'}</p><p>当时响应窗口：${escape(e.before.window?.label||'未确认')}</p>${e.payload.note?`<p>${escape(e.payload.note)}</p>`:''}</details></li>`).join('')}</ol></details>`;
}
function secondWorkspacePage(){
  ...
  return `<section class="second-workspace" id="second-workspace">
    ...
    ${typeof secondRoutesPage==='function'?(state.turn_player===1?`<details class="second-panel"><summary>我方回合路线续算 · 内置练习接入</summary>${secondRoutesPage(doc,readonly)}</details>`:secondRoutesPage(doc,readonly)):''}
    ...
    ${readonly?'':`<div class="second-editors"><details class="second-panel" open><summary>记录卡牌变化</summary>
