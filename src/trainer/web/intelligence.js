...
const intelNames={endboards:'终场标记',handtraps:'手坑标记',breakers:'解场标记',opponents:'对手卡组',records:'断点管理',opening:'起手分析'};
...
function intelCardSummary(item){if(item.managed_by==='card-annotations')return `<span class="intel-list-summary">${escape(item.note)}</span><small>${item.effect_keys.length} 个用途效果 · ${escape(item.classification_basis)}</small>`;const effects=Object.values(item.effects||{}),pending=item.unmatched_effects||[],hand=intelIsLibrary(intelUI.tab),count=[...(hand?[]:[item]),...effects,...pending].reduce((sum,v)=>sum+intelNotes(v).filter(n=>n.text.trim()).length,0);return `<span class="intel-list-summary">${escape(item.note||`已标注 ${effects.length} 个效果`)}</span><small>${hand?`${effects.length} 个效果 · ${count} 条效果备注`:`${count} 条备注 · ${item.sources?.length||0} 个来源`}${pending.length?` · ${pending.length} 个效果待核对`:''}</small>`;}
...
function intelFilters(prefix,{records=false}={}){
  ...
  const abilities=records||prefix==='intel-pick'&&intelUI.data?.annotation_sync?'':`<label class="intel-filter">效果能力<select id="${prefix}-effect_tag">${capabilityFilterOptions()}</select></label>`;
  const candidates=!intelUI.data?.annotation_sync&&prefix==='intel-pick'&&['handtraps','breakers','endboards'].includes(intelUI.tab)?`<label class="intel-filter">用途候选<select id="${prefix}-purpose_candidate"><option value="">全部卡牌</option><option value="${intelUI.tab}">按标注查找${intelNames[intelUI.tab]}候选</option></select><small>候选需人工确认，未标注卡可在全部卡牌中选择。</small></label>`:'';
...
function intelControls(){
  ...
  for(const key of Object.keys(intelNames))$(`[data-intel-count="${key}"]`).textContent=key==='opening'?'本地':key==='opponents'?(intelOpponentUI.data?.decks.length??'只读'):Object.keys(d[key]).length;
  $('#intel-actions').innerHTML=d.annotation_sync&&!records?'<button data-intel-action="sync-annotations">重新同步标注</button><button data-intel-action="browse-annotations">维护卡片标注</button><button data-intel-action="add">补充未标注卡</button>':`${!hand&&!records?'<button data-intel-action="sources">汇总方案标记</button>':''}${hand?'<button data-intel-action="import-staples">导入常用分类</button>':''}${records?'<button data-intel-action="import-matchups">导入主流对策</button>':''}<button class="primary" data-intel-action="add">${records?'新增断点':'添加卡牌'}</button>`;
  ...
  if(d.annotation_sync&&!records){
    $('#intel-controls .intel-group-actions')?.remove();
    $('#intel-controls').insertAdjacentHTML('afterbegin',`<p class="capability-filter-note">由统一标注自动生成；${d.annotation_sync.unavailable_codes.length} 张标注当前不可用于分类。旧资料已按版本备份。</p>`);
  }
...
function intelList(){
  ...
  if(records)entries=entries.filter(record=>intelMatchupMatches(record,{...f,topic:$('#intel-topic').value},d));
...
function intelEditor(){
  ...
  const recordRead=kind==='records'&&(intelUI.recordMode==='read'||intelUI.recordMode===null&&!!draft?.research);
  ...
  const entry=`${kind}:${draft.id??draft.code??'new'}${kind==='records'?':'+(recordRead?'read':'edit'):''}`,sameEntry=$('#intel-editor').dataset.entry===entry;
  ...
  if(kind==='endboards'||intelIsLibrary(kind)){
    ...
    if(draft.managed_by==='card-annotations')html+=`<p>${escape(draft.note)}</p><p class="capability-filter-note">${escape(draft.classification_basis)} · 用途、条件与效果直接来自统一标注。</p><div id="intel-managed-effects"></div>`;
    else if(kind==='endboards')html+=intelEndboardEditor(draft,c);
  ...
  if(managed){
    void mountCapabilities($('#intel-managed-effects'),draft.code,{text:draft.desc,purpose:kind,managed:true,effectKeys:draft.effect_keys});
  }else if(kind==='endboards'||intelIsLibrary(kind)){
  ...
  if(kind==='endboards'&&!managed)document.querySelectorAll('#intel-editor details .intel-source').forEach((el,i)=>el.insertAdjacentHTML('beforeend',`<button data-intel-source-plan="${escape(draft.sources[i].plan_id)}">打开来源方案</button>`));
  ...
  if(kind==='records'&&!recordRead&&!d.topics[draft.topic_id]){
...
async function intelSelect(key){if(!await intelDiscard())return;intelClosePicker();intelUI.kind=intelUI.tab;intelUI.draft=intelCopy(intelUI.data[intelUI.tab][key]);intelUI.recordMode=null;intelUI.dirty=false;intelEditor();intelStatus('已载入资料。');}
...
function intelConflictHtml(value,kind,latest){
  ...
  if(kind==='records'){
    ...
    const research=value.research?['summary','recognition','priorities','warnings','metagame'].map(key=>value.research[key]?`<p>${escape({summary:'一句概要',recognition:'识别线索',priorities:'优先打断',warnings:'常见误区',metagame:'样本依据'}[key])}：${escape(value.research[key])}</p>`:'').join(''):'';
    return `<strong>${escape(value.title)}</strong><p>${escape(value.note)}</p>${research}${steps}`;
  }
...
async function intelAction(action){
  if(action==='record-edit'||action==='record-read'){
    if(intelUI.kind!=='records')return;intelClosePicker();intelUI.recordMode=action==='record-read'?'read':'edit';intelEditor();
    intelStatus(intelUI.dirty?'未保存输入已保留。阅读展示当前草稿，仍需保存资料。':action==='record-read'?'已切换到阅读。':'正在编辑资料。');return;
  }
  if(action==='import-matchups'){
    if(!await intelDiscard())return;
  ...
  if(action==='accept-latest'){
    ...
    if(intelUI.draft?.id&&!intelUI.data[intelUI.kind==='topic'?'topics':intelUI.kind==='folder'?'folders':intelUI.kind]?.[intelUI.draft.id]){
      if(intelUI.kind==='records')intelMatchupDetachResearch(intelUI.draft);
      delete intelUI.draft.id;
    }
    intelControls();intelList();intelEditor();intelStatus('输入已保留。点击保存将用当前输入更新该资料。');return;
  }
  if(action==='sync-annotations')return intelWrite('annotations.sync',{});
  if(action==='browse-annotations')return switchModule('cardanno');
  ...
  if(action==='save'){const op={endboards:'endboard',handtraps:'handtrap',breakers:'breaker',records:'record',folder:'folder',topic:'topic'}[intelUI.kind],researchRecord=intelUI.kind==='records'&&intelUI.draft.research,id=intelUI.draft.id;const saved=await intelWrite(op+'.save',intelUI.draft);if(saved&&researchRecord){intelUI.kind='records';intelUI.draft=intelCopy(intelUI.data.records[id||intelUI.data.saved_id]);intelUI.recordMode='read';intelEditor();}return saved;}
  ...
  if(['folder-new','topic-new','add'].includes(action)){
    ...
    if(intelUI.tab==='records'){const topic=$('#intel-topic').value||Object.keys(intelUI.data.topics)[0];if(!topic)return intelStatus('请先新建一个适用主题。');intelUI.kind='records';intelUI.recordMode='edit';intelUI.draft={title:'',topic_id:topic,note:'',steps:[intelNewStep()],status:'待补充'};intelEditor();return;}
...
$('#intelligence').addEventListener('click',run(async e=>{
  ...
  if(b.dataset.intelTab){if(!await intelDiscard())return;if(typeof openingIntel!=='undefined')OpeningView.invalidate(openingIntel);intelClosePicker();intelUI.tab=b.dataset.intelTab;intelUI.draft=null;intelUI.kind=null;intelUI.dirty=false;intelShell();return;}
