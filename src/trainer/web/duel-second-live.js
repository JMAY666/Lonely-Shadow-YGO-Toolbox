...
function secondLivePage(doc,readonly){
 ...
 <p>仅覆盖当前手牌、可见区域和 LP。牌库只读取张数，对手隐藏卡不读取身份；阶段、素材、完整连锁及次数仍缺失。两次快照之间的动作不会被猜成费用或无效。</p>
 ${readonly?'':`<button type="button" id="second-live-read">读取公开资源</button>`}
 ${s?`<p id="second-live-freshness"></p><p>第 ${s.turn} 回合 · LP ${s.lp[0]} / ${s.lp[1]}。我方手牌：${s.cards.filter(c=>c.controller===0&&c.location===2).map(c=>escape(secondName(doc,c.code))).join('、')||'无'}</p>${groups}
 <details><summary>本次可见卡牌与范围</summary><ul>${s.cards.map(c=>`<li>${c.controller?'对手':'我方'}${SecondDuelModel.zones[c.location]} · ${escape(secondName(doc,c.code))}${[4,8].includes(c.location)?` · 第 ${c.sequence+1} 格 · ${c.position&5?'表侧':'里侧'}`:''}</li>`).join('')}</ul><p>缺失：${s.missing.map(escape).join('；')}。</p></details>
 ${readonly?'':`<form id="second-live-apply"><label><input type="checkbox" name="confirmed" required> 已核对；更新当前已覆盖资源，保留原始起手及旧人工历史。牌库只保留张数，阶段与规则需重新核对。</label><button type="submit" ${SecondLiveModel.canApply(doc,p)?'':'disabled'}>再次读取核对并采用</button></form>`}`:'<p>先读取，再与当前记录核对；不会直接覆盖人工填报。</p>'}
