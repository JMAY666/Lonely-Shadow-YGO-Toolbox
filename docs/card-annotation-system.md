...
- **进行中／未完成**：全卡库逐卡逐效果标注（当前 14,716 张非衍生物中，2,682 张已标注，12,034 张尚未标注），继续以完整系列分阶段核对；完整用途覆盖与动态局面推断仍不由静态标注承担。样本之外的任何能力查询结果均受「已标注范围」限制，不宣称全库准确。
...
- `actions.extra_reveal`明确公开、展示或确认自己／对方额外卡组的全部或部分卡；必须有`from_zones`且只能为`extra`／`opponent_extra`。数量、查看者和范围在处理说明保留，归入`etag:deck-look`。正例为银河百式②确认对方额外、地狱裂魔①展示自己额外1只融合；反例为主卡组翻开（`deck_reveal`）或把额外怪兽加入手卡。旧`deck_reveal`资料保持兼容，未借本轮自动改写其区域或裁定。
- `usage_limits.battle_step_once`表示每个合格战斗步骤一次（No.9②），不同战斗步骤仍可；`name_duel_once`表示同名指定效果一场决斗只使用一次（否定人②）；`chain_once`表示同一实体卡指定效果同一连锁只发动一次（No.18①）。不得统一改成`soft_opt`或`name_soft_opt`，也不执行实时计数器。
- `burn`／`heal`可以用`amount_rule: {text, evaluated_at: "resolution"}`保存动态伤害／回复计算依据，不能同时写固定`amount`，不能附执行字段。正例为黏土守卫侠①按对方场上卡数乘400、No.XX②按对象原本攻击力回复；反例为支付一半LP的费用、无效失败仍假定伤害成功。沿用`set_lp`的动态数值边界，界面与引擎适配不执行该文本表达式。
- `change_attribute.mode`可明确为`add`或`replace`。对极英雄①使用`add`追加光属性，保留原暗；未填`mode`的旧资料保留原行为。追加光不能解释成替换暗，无效与作用区域仍按原来源判断。
- `attach_material`定义扩展为把指定卡叠放为超量素材，分别保留来源、去向本体和随同材料的差异。No.9③从墓地附两张、No.90③从卡组附一张与No.75把本体连材料转移有不同来源；发动费用取除不能标成附素材／取除的处理能力。

查询只支持实际服务字段。效果类别、发动区域等尚无搜索条件的项目用`semantic_cases`冻结，批次门禁拒绝未知查询字段，避免API忽略参数后仍宣称筛选已测试。新处理不替代现场发动合法性或引擎规则验证。

## 1.49.8 处理数量、基本分减半与事件次数

`structure.processing`的每个处理项可使用`count_rule`保存动态数量，字段限定为`mode`、`text`、`evaluated_at`以及`up_to`模式必需的非负整数`minimum`。`mode=exact`不含`minimum`；`evaluated_at=activation/resolution`明确发动或处理时确定，不能与`count/min_count/max_count`混用。正例为天空侠①破坏上限按处理时其他己方表侧HERO数、星云新宇侠①按处理时对方场上卡数抽卡；反例为固定抽2或动态对象数量。对象仍使用原`targeting.count_rule`的发动时点协议，不把处理时选卡伪装成取对象。

常量数量范围用同一处理项的`min_count/max_count`，必须成组，不与固定`count`或动态规则混用。最小值为非负整数，有限上界不得小于下界，`max_count:null`才表示无固定上界；布尔值拒绝。重铠光子龙①的1至2张装备卡与恐惧人①可选叫出1至2只分别保存常量闭区间，不能称作动态无上界。可选是否进行由`optional`与逐卡条件另行说明。详情、只读能力事实及新报告快照保存数量依据与取值时点，旧快照不补写、不重新求值，程序不执行表达式。

`actions.halve_lp`将`recipient=self/opponent/both`玩家的处理时当前基本分减半，必须有`evaluated_at=resolution`、`is_effect_damage=false`、`is_payment=false`及选择说明，禁止固定／动态伤害数值、卡片来源去向、数量或另一承受者规则。正例为No.85掷出1的自己减半、CNo.104无效后可选随机送墓成功才对方减半；反例为支付一半LP费用或给予当前LP一半的效果伤害。它不自动赋予伤害／回复TAG，不执行真实LP或端数计算。[No.85官方补足](https://www.db.yugioh-card.com/yugiohdb/faq_search.action?cid=10707&ope=4&request_locale=ja)与[CNo.104官方补足](https://www.db.yugioh-card.com/yugiohdb/faq_search.action?cid=10659&ope=4&request_locale=ja)分别给出分支和前后成功条件。

新增次数`attack_declaration_once`（No.96，每个符合条件攻击宣言一次）、`damage_calculation_once`（电光皇，每个符合条件伤害计算一次）、`name_apply_once`（源数创造，同名每回合只能适用一次）。不得替换成`soft_opt`、`battle_step_once`、同名使用或同名卡发动次数；这些取值只记录静态边界，不执行实时计数。现有TAG ID保持不变。

`return_to_field`除原`from_zones=['banished']`外，支持独立的`['opponent_banished']`，仍须明确场上去向、原表示、返回时点及`delayed`，保持`creates_chain=false/counts_as_special_summon=false`。不能把两种来源不加区分地混在一个处理项，更不能默认转移控制者。正例为银河眼辉光子龙②暂时除外的对方怪兽到原控制者区域返回；反例为从除外状态特殊召唤。原有记录保持原含义，不自动迁移。

多个真实固定获赋效果继续使用各自`grant_effect.granted_effect`，本次CNo.62的两个●分别登记免疫与增攻，不合成一个效果。不虚构获赋来处理单一无编号段内的独立效果。玩家抽卡与原持有者检索也分别绑定各自处理项的来源／去向；对方卡组到自己手卡的错误组合有实际反例，不借另一处理的区域。
