...
默认进入系列卡牌夹，按 `Intl.Collator('zh-Hans-CN')` 的中文名称顺序分页（每页 24 个）。全卡库浏览包含未标注卡；进入系列后仍沿用每页 30 张的卡片查询。筛选整体及原卡图默认收起，展开原图时才请求图片；切换卡片重置原图收起状态。彩色效果 TAG 同时保留中文名称、分类分组和选择勾号。额外卡组标志严格读取融合／同调／超量／连接类型位；仪式、灵摆、调整各自另行显示，不因灵摆类型而直接视为额外卡组怪兽。
