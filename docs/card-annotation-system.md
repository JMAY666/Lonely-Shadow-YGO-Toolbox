...
- 1.48.0 通过统一只读能力服务接入卡片详情、三类标记、构筑与 TAG 检索、起手分析、断点与实战参考。引用按卡号＋效果键＋卡文指纹定位；个人用途、原有规则判定和历史方案仍独立保留，详见[跨模块能力引用](card-capability-integration.md)。

## 9. 当前状态与后续阶段

- **已完成**：规范与实现（分段、校验、覆盖清单、组合查询、自动草稿、个人层）、19 个当前 TAG 与 1 个兼容标签的词表；首批 11 张样本、第二批 35 张引擎审计清单、第三批 62 张手坑／解场，以及后续四次完整系列阶段新增 100、94、100、96 张。第七批闪刀／半龙女仆／斩机／军贯的来源和异画核对见[逐卡清单](card-annotation-batch-2026-09-25-phase4.csv)，上一批见[1.47.6 清单](card-annotation-batch-2026-09-25-phase3.csv)。
- **进行中／未完成**：全卡库逐卡逐效果标注（当前 14,716 张非衍生物中，498 张已标注，14,218 张尚未标注），继续以完整系列分阶段核对；完整用途覆盖与动态局面推断仍不由静态标注承担。样本之外的任何能力查询结果均受「已标注范围」限制，不宣称全库准确。
