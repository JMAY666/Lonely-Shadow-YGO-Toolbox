...
日期：2026-09-23。版本：0.1。状态：供继续讨论的完整建议稿，未授权实施第 2—5 点。

用户已确认以 YGOPro 为首个客户端，沿用实战辅助定位：跟随真实对局给出操作建议，由用户在游戏中操作。第一点的起手分析已在 1.42.1—1.42.2 实现，实际范围见[使用说明](going-second-opening.md)与[验收记录](verification-1.42.2.md)。本次仅完善计划和讨论记录，不修改程序、不接入实时读取、不运行推演或恢复 AI 学习计划。

## 1. 共同原则与当前起点

沿用前面确定的思路：先理解系列如何运转，再把卡片作用与具体路线、当前资源联系起来；采用本地资料、规则和方案归纳。人工可调整确定的通用用途与展示偏好，局部推导只读并允许备注。一卡动、补点和相对废件继续以方案及目标为依据。

以下“推荐答案”和数量、时延参数都是本稿建议，不自动变成用户已确认的要求。本稿仍处于计划阶段，后续是否实施按用户届时指令确定。
...
此表是未来工作安排，本次不执行。建议每阶段完成实际验收后再进入下一阶段，不按日期承诺全部卡组覆盖。
...
本次文档维护检查只核对内容、内部链接、与当前已实现能力的界限和 Git 差异；未执行新增功能测试，未改变应用版本。
