...
仍只对应上述同一镜像指纹。参照分发包内 `native-sources.zip` 固定客户端源码（提交 `30f173a678bdade61a7b37f736dec1771e9afef9`）的 `ClientAnalyze`、`SendResponse`、`ClearChainSelect` 和结构定义，结合该镜像指令中的偏移写入／容器访问与实机窗口交叉核对。

| 字段 | 本镜像偏移／布局 | 用途与限制 |
| --- | --- | --- |
| 当前消息类型 | `G + 0xe28`，u16 | 必须是当前普通连锁选择消息；不能只看旧缓存 |
| `chains` | `G + 0x1610`，vector，条目 64 字节 | 已入列公开条目；不遍历条目内的对象集合 |
| 链条目 | 卡号 +24、描述 +28、玩家 +32、区域 +36、序号 +40、处理开始 +44 | 非场上来源不输出序号；源码名 `solved` 实际在处理开始设置，不能标作已完成 |
| 临时 `current_chain` | `G + 0x1758` | 仅用于研究核对；消息 70 写入期间不把临时条目当作完整列表发布 |
| 候选卡指针列表 | `G + 0x1580` | 与已知我方区域中的同一卡片对象核对 |
| 候选描述／标记列表 | `G + 0x15c8`，每项 8 字节 | 与消息中的描述及普通／必选标记一致才保留 |
| 可见选择提示 | `read_u64(G + 0x2ed8) + 0xa8` | 提示区未显示时不认证支持的我方选择 |
| 卡片可选／命令标记 | `ClientCard + 0x7f`／`+ 0xec` | `SendResponse` 清除后拒绝旧缓存，命令须含 `COMMAND_ACTIVATE` |

消息 16 的固定头为 12 字节，每项 14 字节；地址玩家先转换为我方视角，确认属于我方后才读候选范围。先核对条目所属区域，再只读已知且受支持的我方卡片身份／描述；牌库、未知来源、操作／重置与素材候选不读身份载荷。普通位置的第四字节是表示信息，不以“必须为零”错误排除；`specount=0x7f` 的诱发选择不作为普通交康窗口。

消息 70／74 的列表更新期标为更新中。消息 72 开始处理；73 不提供可靠逐条完成标记；75／76 的动画与缓存不被解释成已确认的两类无效。列表、消息、区域、描述与可选标记一起参与读取前后复核，以及两次完整快照的一致性检查。

用户实机核对两个手坑选项及一个连锁 1 的公开来源／我方选项；发动后手牌数与候选变化已观察。单槽缓存和轮询不提供完整事件、精确窗口身份、漏读恢复或规则权限；历史相同内容的新窗口仍不能自动识别。源码／镜像研究保存在 `.local/ygopro-p23-research/`，真人原始资料及确认记录保存在 `.local/ygopro-p23-live/`，均不提交。详见[覆盖说明](bo1-second-chain.md)与[验证记录](verification-1.36.0.md)。
