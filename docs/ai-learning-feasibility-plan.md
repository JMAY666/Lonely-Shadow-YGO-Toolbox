...
版本：v7，2026-09-21。依据[修复后完整标定与预算复核](ai-learning-p3-revalidation-results.md)修订。**P2 的 200 家族与划分不变；修复后二进制完成同一批 20 个训练家族的 40 次 B1/T0 执行，完整重放与无损恢复通过，T0 仍为 15/20，B1 仍为 13/20。新实测的 200 家族保守时间为 4.46 小时，超过原 4 小时门槛；累计 worker 约 97.10 分钟，剩约 22.90 分钟，完整 40 家族验证保守预测约 53.51 分钟。旧版 24/40 部分验证及全部失败证据保留，新版完整验证尚未启动，继续停止扩大采样。** P0A 与限定 P0B/P1 保留原结论；整体仍为部分可行。
...
4. `T0-resource-v7` 保留结算中的可见起动潜力，并在原前三候选之外比较合法停止动作，仍扣 24 节点预算。20 个训练回合全部结束、15 个达标；部分验证的 17 个达标不能代替完整 40 家族结论。学生 GPU 训练短测仍不能代表整体提速。
5. 200 家族按 120/40/40 冻结，ZIP/gzip 无损证据与旧记录保留。修复后同版本完整标定已完成，保守新增空间约 4.35 GiB、时间约 4.46 小时。旧验证登记继续保留，新版验证尚未启动；200 家族时间门槛和剩余 P2/P3 worker 预算均未通过，因此不启动全量或正式采样。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | 修复后新增 40 次完整训练标定，重放与归档审计通过；此前 48 次部分验证、12 次恢复回归及失败记录保留 | 修复后完整验证与正式数据逐条审计 |
| H2 | 教师数据比现有基线有价值 | 修复后训练标定仍为 T0 15/20、B1 13/20；旧版验证仅完成 24/40 对，其中 T0 17、B1 14，无完整选型结论 | 新版本完整验证的预登记目标、共同支持范围及成本门槛 |
...
| P3 教师质量 | 修复后完成同版本 20 家族标定，T0 15/20、B1 13/20；200 家族保守成本 4.46 小时，旧版部分验证仍为 24/40 | 解决成本与累计预算，再新登记全部 40 家族验证；不能混用部分结果 | 同版本标定完成，预算及完整选型未通过 |
...
原 P2–P6 的 6–12 个有效工作日、P7/P8 的 2–4 天只作历史工作量规划，不是计算耗时承诺。旧版完整验证因死锁中止；修复后完整训练标定已完成，但新的保守成本和剩余 worker 预算均未通过，不能按旧排期直接采样。P1、历次 P3 标定、部分验证和修复诊断的成本分别记录，不互相替代。
...
200 家族小试的本节冻结工作已执行，见 [P2 协议](ai-learning-p2-protocol.md)。旧版验证完成 24/40 对后中止，修复后完整验证尚未启动，留出继续封存，正式 2,000 家族尚未生成。分组由起手与预先定义的结构条件决定，不按本轮教师结果改变。
...
已完成原标定、[纠偏重测](ai-learning-p3-correction-results.md)与[资源优化](ai-learning-p3-resource-results.md)：T0 训练标定达标 15/20，B1 13/20，保留上一版全部 13 个达标家族。固定配置为 24 节点、12 层、2 秒、束宽 2，前三候选之外可额外比较合法停止动作，仍扣同一预算；未知随机结果仍在评分前停止。随后按[预登记](ai-learning-p3-validation-protocol.md)验证，完成 24/40 对后发生原生死锁，修复后两家族的 12 次回归通过，完整验证仍未通过。原始 40 家族分母、失败记录与旧版本保存，不能从部分验证或修复样本宣称独立策略收益。

修复后的[完整训练标定](ai-learning-p3-revalidation-results.md)再次运行相同 20 家族、40 次 B1/T0，教师和基线达标数不变，完整重放与归档审计通过；新的保守时间为 4.46 小时/200 家族。它补齐同版本工程证据，未补齐独立教师选型；必须先解决预算再登记新的完整验证。
...
用户约束为零付费调用。已有环境、模型与证据继续保留；下列上限不因阶段完成而放宽。修复后完整训练标定给出 200 家族保守 4.46 小时、含恢复预留约 4.35 GiB；时间超出原 4 小时门槛，空间通过。旧版的 3.76 小时不能代替当前实测。新会话原始字节用经恢复核对的 ZIP/gzip 保存，被占用的重复文件保留并计入空间。时间上限是止损值，不是承诺；不能用部分结果或重新启动重置累计预算。
...
| 下一执行包 | P2/P3 worker 累计限 2 小时；已计约 97.10 分钟，余约 22.90 分钟，低于修复后完整验证保守预测 53.51 分钟；预算待解决，留出集继续封存 |
...
**修复后同版本完整训练标定与预算复核已完成，独立教师选型仍未完成，不能进入 P4。下一执行包是解决预算并重新登记全部 40 家族验证。**

1. 保留 P2、B1、全部分母、随机条件、旧验证登记和各失败/诊断批次。教师固定 `T0-resource-v7`，不根据部分验证临时修改后拼接结果。
2. 复用修复后 20 家族标定、完整重放及新旧前缀审计；新执行必须绑定实际原生与策略哈希。资源或策略改变时重做受影响核验，旧登记不能覆盖。
3. 当前余约 22.90 分钟，完整验证保守预测 53.51 分钟，差额约 30.61 分钟；200 家族预测 4.46 小时也超过 4 小时门槛。保持原预算须继续降低成本；仅提高 P2/P3 时间上限不能解除小试成本门槛。
4. 已准备待确认预算方案：P2/P3 累计上限改为 3 小时、200 家族累计上限改为 5 小时，其余资源与质量门槛不变。该方案尚未批准或生效，程序仍执行原 2/4 小时限制；不得因本报告而自动启动验证或采样。
5. 预算满足后，以新登记保存完整 40 家族、修复后版本、标定来源和实际累计账本；八批各五家族，可靠性失败即停止。旧版 48 次部分验证和 12 次恢复回归不混为新版完整结论，holdout 继续封存。
6. 存储继续逐字节恢复审计，全部异常轨迹、归档、恢复点与环境保留。完整预登记验证及保守成本同时满足后才进入 P4；正式学生训练、LLM 对照、2,000 家族、能力更新与产品接入继续等待各自门槛。

本次交付为同版本 40 次真实执行、完整原生证据恢复审计、可重复运行的修复前后轨迹与预算复核入口，以及明确的停止原因与待确认预算方案。本轮没有打开新版验证或 holdout，没有训练学生、调用 LLM 或改变正式产品流程。
...
- [资源与时间优化](ai-learning-p3-resource-results.md)、[验证预登记](ai-learning-p3-validation-protocol.md)、[验证中止与死锁修复](ai-learning-p3-validation-results.md)：v6 的训练标定、部分验证、运行失败、确定性锁竞争回归与累计预算依据。
