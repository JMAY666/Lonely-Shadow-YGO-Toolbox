...
> **已停止并封存，2026-09-21，用户主动决定。** 最后实验提交为 `ae8ee353827be47412c2f907a8144b6a3a14fa68`。以下 v8 内容作为停止时的历史资料保留，原预算、命令和后续事项均不再执行；只有用户明确要求恢复本计划才重新安排工作。完整封存内容、提交与推送核对见[封存与历史回溯](ai-learning-archive.md)。计划整体未完成。

版本：v8，2026-09-21。依据[完整验证结果](ai-learning-p3-validation-v8-results.md)修订。**用户已批准 P2/P3 累计上限 3 小时、200 家族成本上限 5 小时。修复后二进制的全部 40 个验证家族、80 次执行完成并通过重放/原生日志/归档审计；T0 达标 29/40，B1 26/40，净增 3 个，满足小试质量门槛，但 95% 配对区间为 −5 至 +20 个百分点。验证实测的 200 家族保守成本为 5.112 小时，超过已批准的 5 小时门槛，因此整体放行未通过，P4 暂缓。累计 worker 约 118.01 分钟，余约 61.99 分钟，旧版登记与失败证据保留。** P0A 与限定 P0B/P1 保留原结论；整体仍为部分可行。
...
4. 教师仍为 `T0-resource-v7`，搜索与质量标准不变。训练标定 15/20、B1 13/20；修复后完整验证为 T0 29/40、B1 26/40。共同支持覆盖全部 40 家族，增加的达标数来自资源紧张子集；区间覆盖 0，不视为正式策略收益已证明。
5. 200 家族按 120/40/40 冻结，旧登记原样保留，新登记为 `validation-registration-v8.json`。完整验证保守新增空间约 3.90 GiB，通过当前空间门槛；保守时间约 5.112 小时，未通过 5 小时门槛。继续停止 P4 扩大采样，优先在既有训练侧解决成本。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | 修复后 80 次完整验证通过重放、原生日志与归档恢复；旧失败和恢复记录保留，331 个搜索窗口隐藏信息替换通过 | 正式采样仍需逐条审计，当前不证明任意场景可靠 |
| H2 | 教师数据比现有基线有价值 | 完整验证 T0 29/40、B1 26/40，小试质量方向通过；95% 区间 −5 至 +20 个百分点，成本门槛未通过 | 解决成本后再按协议推进；正式收益仍需封存测试证据 |
...
当前基础包括[学生训练与执行试验](../experiments/ygo_learning/README.md)、[P0B/P1 结果](ai-learning-p0b-p1-results.md)、[P2 协议](ai-learning-p2-protocol.md)、[P3 标定结果](ai-learning-p2-p3-results.md)、[P3 纠偏结果](ai-learning-p3-correction-results.md)、[资源与时间优化](ai-learning-p3-resource-results.md)、[验证与死锁修复](ai-learning-p3-validation-results.md)、[修复后完整标定](ai-learning-p3-revalidation-results.md)、[原模型试验](../experiments/ygo_agent/README.md)及[模块化文档](modular.md)。P0A 的历史证据来自 `2234928`，后续批次保留单独的源码与资源清单。正式运行仍须冻结实际代码、特征、引擎与资产版本。
...
| P2 冻结实验协议 | 200 家族小试及 120/40/40 划分已冻结；2,000 家族正式协议未启动 | 不同阶段和训练/验证/测试无泄露，失败分母固定 | 小试冻结已完成 |
| P3 教师质量 | 修复后完整 40 家族验证与 80 次审计完成；T0 29/40、B1 26/40，小试质量方向通过 | 200 家族保守成本 5.112 小时须降至已批准的 5 小时内；正式质量仍未验证 | 完整验证完成，成本未通过，P4 暂缓 |
...
原 P2–P6 的 6–12 个有效工作日、P7/P8 的 2–4 天只作历史工作量规划，不是计算耗时承诺。修复后完整验证已完成，小试质量方向通过，但验证实测保守成本超限，不能按旧排期直接采样。P1、历次标定、旧部分验证、修复诊断和完整验证的成本分别记录，不互相替代。
...
200 家族小试的本节冻结工作已执行，见 [P2 协议](ai-learning-p2-protocol.md)。旧版验证完成 24/40 对后中止；修复后使用独立登记完成全部 40 对，留出继续封存，正式 2,000 家族尚未生成。分组由起手与预先定义的结构条件决定，不按本轮教师结果改变。
...
已完成原标定、[纠偏重测](ai-learning-p3-correction-results.md)与[资源优化](ai-learning-p3-resource-results.md)：T0 训练标定达标 15/20，B1 13/20，保留上一版全部 13 个达标家族。固定配置为 24 节点、12 层、2 秒、束宽 2，前三候选之外可额外比较合法停止动作，仍扣同一预算；未知随机结果仍在评分前停止。按[原预登记](ai-learning-p3-validation-protocol-v1.md)运行的验证在 24/40 对后发生原生死锁，旧分母、失败与诊断记录继续保留。

修复后的[完整训练标定](ai-learning-p3-revalidation-results.md)再次运行相同 20 家族、40 次 B1/T0，教师和基线达标数不变。用户批准 3/5 小时预算后，按 [v8 预登记](ai-learning-p3-validation-protocol.md)完成全部 40 个验证家族：T0 29/40、B1 26/40，共同支持 40 家族净增 3 个；80 次执行与恢复审计、331 个搜索窗口隐藏信息替换通过。小试质量方向通过，区间仍覆盖 0；验证成本外推为 5.112 小时/200 家族，超出 5 小时门槛，完整放行未通过，详见[结果](ai-learning-p3-validation-v8-results.md)。
...
experiments/ygo_learning/
  ...
  protocol_p2.py                    # 已冻结的 200 家族小试协议
  teacher_p3.py / module_proposals.py # 限定 T0 搜索与公共动作提案
  calibration_p3.py                 # 20 个训练家族配对标定和累计预算
  budget_policy.py / validation_p3.py # 经批准的版本化预算、旧登记读取和新验证登记
  evidence_p3.py / session_archive.py # 收集器与原生文件的无损归档/恢复
  report_p3.py / report_p3_evidence.py # 原始/归档兼容审计与保守成本
  revalidation_p3.py                 # 修复后二进制、完整前缀与剩余预算的离线复核
  README.md                         # 证据、命令与限制
```

以下正式实验职责仍待开发或扩展；已有 P1、P2 小试划分与限定 P3 实现可复用，但不能把正式数据划分、独立教师质量或能力更新标为完成：
...
用户约束为零付费调用，已明确批准 P2/P3 累计 3 小时与 200 家族成本 5 小时；其余上限保持不变。完整验证的 200 家族保守成本为 5.112 小时，含恢复预留新增空间约 3.90 GiB；空间通过，时间未通过。训练标定的 4.46 小时不能替代验证集实测。旧原始字节、失败记录和恢复点全部保留，账本不因版本、重试或新会话归零，后续仍不得自动放宽上限。
...
| 下一执行包 | P2/P3 worker 累计限 3 小时；已计约 118.01 分钟，余约 61.99 分钟；完整验证已完成，先在训练侧解决成本，留出集继续封存 |
| 200 家族小试 | 教师/引擎累计最多 5 小时；学生每配置/种子最多 15 分钟、累计最多 1 小时 |
...
此指标来自引擎中的连续展开。另报告受干扰子集表现、支持范围、失误位置、终止/回退原因和耗时；训练准确率、原教师轨迹上的单步命中率只作诊断。78/78 的原探针拟合、423 决策的 98.35% 开发拟合、44/50 真实结束回合、新训练标定的 15/20，以及部分验证的 17/24，均不满足本节的正式质量门槛；最后一个数也不是完整 40 家族的结果。
...
**停止时状态：已完成修复后全部 40 家族验证，小试质量方向通过，成本放行未通过。用户现已停止并封存整个计划，下列成本优化及后续阶段均取消执行，仅保留为历史未完成事项。**

1. 保留 P2、B1、全部分母、随机条件、旧登记、v8 登记和全部失败记录。教师仍固定 `T0-resource-v7`，不能把现有验证结果用于修改策略后宣称新的独立收益。
2. 当前保守成本 5.112 小时，超出 5 小时约 6.73 分钟；配对 p95 加平均开销为 46.010 秒，达到 5 小时需降到 45 秒以内，约降低 2.20%。这只是所需幅度，不证明优化已经实现。
3. P2/P3 已计 118.01 分钟，余 61.99 分钟。后续诊断、重测继续计入同一 3 小时账本；保持每批 30 分钟、8 GiB 内存和 20 GiB 目录限制。不得把报告中纳入的成本移出账本以凑门槛。
4. 先用已登记训练家族定位前缀、通信、归档等具体开销；优化不能省略实际选择、最终完整重放、无损恢复或失败证据。策略/引擎/运行器身份改变时独立冻结来源并做同条件前缀回归。
5. 成本重验先另登记版本、范围和停止规则，保留本轮 40 家族结论；不得挑选更快的重试替换当前成本，也不能将旧版部分验证混入新结果。holdout 继续封存。
6. 只有可靠性、已登记质量及保守成本同时满足才进入 P4。正式学生训练、LLM 对照、2,000 家族、能力更新与产品接入继续等待各自门槛。

本次交付为版本化预算策略、v8 独立登记、完整 80 次执行与审计、331 窗口隐藏信息替换、固定 10,000 次配对重采样，以及通过/未通过分项结果。没有扩大到 P4、打开 holdout、训练学生或调用 LLM。
...
- [P2 小试协议](ai-learning-p2-protocol.md)、[原 P3 标定](ai-learning-p2-p3-results.md)及[教师纠偏与原生归档](ai-learning-p3-correction-results.md)：v4/v5 修订的实测依据，包含固定家族配对、失败记录、完整恢复、隐藏信息替换及停止扩展结论。
- [资源与时间优化](ai-learning-p3-resource-results.md)、[原验证预登记](ai-learning-p3-validation-protocol-v1.md)、[验证中止与死锁修复](ai-learning-p3-validation-results.md)：v6 的训练标定、部分验证、运行失败、确定性锁竞争回归与累计预算依据。
- [修复后完整标定与预算复核](ai-learning-p3-revalidation-results.md)：v7 的同版本 40 次执行、完整轨迹比较、保守成本与待确认预算方案。
- [v8 完整验证结果](ai-learning-p3-validation-v8-results.md)：经用户确认的 3/5 小时预算、全部 40 家族结果、置信区间、证据审计与成本停止依据。
