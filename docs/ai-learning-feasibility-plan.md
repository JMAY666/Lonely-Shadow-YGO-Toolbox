...
版本：v2，2026-09-19。依据提交 `2234928` 的学生环境验收修订。**P0A 硬件与训练工具链已通过；下一步为 P0B 引擎吞吐与 P1 数据/执行闭环。** 正式教师质量、未见起手表现、模型更新及产品接入仍未验证，后文门槛不代表已取得的结果。

用户已选择：**本机优先，先验证不产生付费调用的流程**。当前仅执行了下文记录的环境与小样本探针；正式批量训练、后台定时任务、付费 API 和云端 GPU 均未启动。后续按阶段执行，保存实际成本与失败证据。
...
3. 当前约 101 万参数的学生保留为编码、训练和导出回归起点。78 个样本及其三场来源只用于开发回归，不进入正式验证/测试集；训练内全对不作为策略验收。
4. 下一投入重点为动态状态、精确动作、完整重放和学生连续决策；尚未测出引擎与教师瓶颈，不能按训练循环的 55.5 倍差值估算整体提速。
5. 先完成 50 个开发回合，再做 200 个独立家族的小规模完整实验；通过数据和教师门槛后，才进入另行封存的 2,000 家族正式实验。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | 已核对原生回执，正式全量重放未完成 | 同版本重放一致；费用、次数、随机结果和后继窗口均有证据 |
| H2 | 教师数据比现有基线有价值 | 未验证 | 在独立局面上的目标达成率或覆盖改善 |
| H3 | 学生能学到教师能力 | 小样本拟合通过，泛化未验证 | 未见起手和干扰下的连续展开表现 |
| H4 | 本地部署足够轻量 | 模型级短测通过，端到端未验证 | 动态特征、状态读取、通信、结算和模型加载的完整测量 |
...
当前基础以提交 `2234928` 的[学生 GPU 环境试验](../experiments/ygo_learning/README.md)、提交 `17af5b7` 的[原模型试验](../experiments/ygo_agent/README.md)、[适配器](../experiments/ygo_agent/adapter.py)及[模块化文档](modular.md)为参照。复用已冻结环境及证据；正式运行仍须记录实际代码、特征、引擎与资产版本，不混入其他正在进行的功能改动。
...
| 训练 | 独立新学生已完成 78 个样本的 GPU/CPU 训练、恢复和 CPU 导出；旧 TFLite 权重未微调 | 正式数据、独立质量评测、更新及产品接入仍待验证 |
| 硬件路径 | RX 9070 XT 已完成本地 LLM 推理及 PyTorch/ROCm 学生训练环境探针 | 游戏引擎吞吐、大规模训练和完整对局尚未验证；不直接沿用上游 CUDA/JAX 环境 |
...
新学生的 CPU `1.08 ms` / GPU `2.08 ms` 来自预先编码的输入张量，各 30 次稳态测量，CPU 为四线程，含张量传输及返回动作编号。它不含原生状态采集、`NativeInput` 特征准备或应用 IPC，也不能直接与旧模型的 `3.156 ms` 比较。已核对原始报告、源码副本和数据哈希；没有为本次计划修订重新训练。
...
保留阶段编号以衔接已有记录，将 P0 拆成已完成的 P0A 与待完成的 P0B。以下是剩余单人有效工作量估计；P0B 测出引擎/教师吞吐后重估，计算时间另计。环境通过不等于整阶段数据或策略通过。

| 阶段 | 工作与交付物 | 进入下一阶段的门槛 | 初步工作量 |
| --- | --- | --- | --- |
| P0A 环境与训练工具链 | 已完成：GPU/CPU 训练、检查点恢复、CPU 导出与依赖锁定 | 复用证据；只有相关变更才回归 | 已完成，不重做 |
| P0B 实际瓶颈 | 引擎/特征/IPC/重放分项计时，测量采样成本 | 给出 200/2,000 家族的保守时间估计；确认资源预算 | 0.5–1 天 |
| P1 可靠数据与连续执行 | 动态状态、动作/历史契约、引擎重放、CPU 学生运行器 | 60 个机制案例及 50 个开发回合；无未解释的状态/响应偏差 | 2–4 天 |
...
P0B–P6 的核心可行性验证暂估约 9–17 个有效工作日；P7/P8 更新与产品接入另约 2–4 天，总计约 11–21 天。GPU 降低了训练计算成本，尚不能据此压缩数据接口、教师搜索与验收的工作量。P1 后先走 200 家族小试，质量或成本不合格时停止扩展。
...
以当前约 101 万参数的字段嵌入/局面编码/动作评分结构为起点，在 P1 契约 v2 定稿后冻结正式结构与参数量。先修输入和监督，再决定是否扩容；不因显存有余量就增加参数。正式训练使用已验证的 GPU 环境，CPU 仅做必要的数值回归与部署验证，不对每个配置重复完整 CPU 训练。历史截断若丢失当前决策所需信息，应改进状态/历史表达或判为不支持。

当前 `.pt2` 仅是受控 CPU 进程中的部署产物，尚不是工具箱可以直接加载的正式策略包；旧 864 卡词表、固定张量形状和新契约不能无检查混用。当前 78 样本权重不作为已学会游戏王的新基线，而是开发回归文件。
...
以下为待开发的正式实验模块，不能因已有探针就标为完成；文件名可在实现时按项目结构调整，职责与证据要求不变：
...
用户约束为零付费调用。已配置约 4 GiB 的隔离环境继续保留；下列为 v2 剩余实验预算，以 P0B 的真实吞吐再核对。时间上限是止损值，不是承诺耗时；扩大范围须在查看正式测试结果前记录，不能无上限重试。
...
| 下一执行包 | P0B/P1 的新增引擎与数据探测先限 2 小时计算；超限先报告瓶颈，不用缩减证据伪造通过 |
...
此指标来自引擎中的连续展开。另报告受干扰子集表现、支持范围、失误位置、终止/回退原因和耗时；训练准确率、原教师轨迹上的单步命中率只作诊断。当前 78/78 的训练内结果不满足本节任何正式质量门槛。
...
**下一执行包为 P0B＋P1，重点是可靠的连续执行。** 按以下顺序执行：

1. 校验现有环境、导出及原始报告指纹，登记正式引擎/脚本/卡库基线，不重装环境。
2. 完成观察/动作契约 v2 的缺口表；优先补动态属性、效果/素材/次数及历史，明确不支持范围。
3. 实现实验 CPU 学生运行器，先用受控策略核对响应映射，再接入匹配契约的开发学生，以回执驱动历史提交；建立拒答、过期和恢复路径。契约/结构变化时，对重新编码的开发数据做一次小规模 GPU 训练与 CPU 导出回归，不直接加载不兼容的旧学生权重。
4. 运行 60 个引擎机制案例，完整重放来源轨迹；原 78 样本仅作编码回归，不冒充新数据。
5. 运行 50 个全新开发回合，其中 30 个同时采集 P0B 性能数据；保留所有正常结束、策略失败和异常，不只展示成功案例。
6. 输出覆盖/失败分类、重放一致性、特征与引擎耗时、200/2,000 家族预算估计，并作出“进入 200 家族小试 / 先修数据或执行 / 预算内证据不足”的结论。

交付物应至少有：环境/资源清单、契约与覆盖表、可复现开发案例、逐步日志与重放检查、分项性能报告。此执行包不开展正式 2,000 家族采样、不安装新的训练框架，也不把实验入口加入正式界面。若仅修订本计划，不启动上述执行包。

## 11. 参考依据

- [原模型试验及限制](../experiments/ygo_agent/README.md)、[固定资产清单](../experiments/ygo_agent/assets.lock.json)：旧 TFLite 的推理证据，原权重未微调。
- [学生 GPU 环境验收](../experiments/ygo_learning/README.md)、[依赖锁定](../experiments/ygo_learning/requirements-rocm-windows.lock.txt)、[当前学生](../experiments/ygo_learning/student.py)：本次 v2 修订的主要实测依据，包含成功、失败修复及未验证范围。
