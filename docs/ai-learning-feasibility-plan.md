...
版本：v4，2026-09-20。依据 [P0B/P1 实测记录](ai-learning-p0b-p1-results.md)及 [P2/P3 教师标定](ai-learning-p2-p3-results.md)修订。**P2 已冻结 200 个小试家族；P3 已完成其中 20 个训练家族的 B1/T0 对照，40 次运行全部重放通过，但 T0 达标 10/20、B1 达标 13/20，扩展预算未通过。** P0A 与限定 P0B/P1 保留原结论。正式教师质量、未见起手收益、能力更新及产品接入仍未验证，整体结论为部分可行。
...
4. 已测出主要时间在引擎推进、渲染和重放；GPU 训练 400 次更新约 6.76 秒，但不能据此估算整体加速。当前 T0 的对手响应边界截断、短期场面评分和两次策略循环需要先修正。
5. 200 家族按 120/40/40 冻结；20 个训练家族标定已完成。按当前 B1＋T0 完整对照外推，200 家族保守耗时约 7.08 小时，超过 4 小时；平均新增完整证据约 8.62 GiB，也超过约 8.30 GiB 的剩余额度。优化并重测前不启动小试全量或正式采样。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | P1 的 115 个会话通过；P3 40 次标定运行的完整重放与归档恢复审计通过 | 正式教师批量数据仍须逐条重放与来源审计 |
| H2 | 教师数据比现有基线有价值 | 20 个训练家族中 T0 10/20，B1 13/20；未显示改进，独立质量未验证 | 修正教师后，在独立局面验证目标达成率或覆盖改善 |
...
当前基础包括[学生训练与执行试验](../experiments/ygo_learning/README.md)、[P0B/P1 结果](ai-learning-p0b-p1-results.md)、[P2 协议](ai-learning-p2-protocol.md)、[P3 标定结果](ai-learning-p2-p3-results.md)、[原模型试验](../experiments/ygo_agent/README.md)及[模块化文档](modular.md)。P0A 的历史证据来自 `2234928`，后续批次保留单独的源码与资源清单。正式运行仍须冻结实际代码、特征、引擎与资产版本。
...
| P3 教师质量 | T0 原型与 20 个训练家族标定完成；本版未通过收益方向及扩展预算 | 先修响应边界、评分、循环和存储，再在验证集选教师 | 标定完成，质量评测未通过/未执行 |
...
原 P2–P6 的 6–12 个有效工作日、P7/P8 的 2–4 天只作历史工作量规划，不是计算耗时承诺。当前教师质量及扩展预算未通过，先修复并重新标定，不能按旧排期直接采样。P1 的 50 回合成本和 P3 的 20 家族配对成本分别记录，不互相替代。
...
已完成限定 T0 标定，见 [实测结果](ai-learning-p2-p3-results.md)：20 个训练家族的 B1/T0 配对均可完整重放，但本版 T0 低于 B1，且保守采样预算未通过。当前搜索每窗只展开前 3 个候选，在对手响应或未知随机结果处停止；公共来源仅作精确起手匹配，尚未成为可泛化模块提案。以下正式教师要求继续保留，本轮不视为 P3 独立质量门槛通过。
...
用户约束为零付费调用。已有环境、模型与证据继续保留；下列上限不因阶段完成而放宽。P3 已测 20 个训练家族，200 家族的保守时间与完整证据成本仍超预算；收集记录压缩恢复通过，但未替代原生 JSONL 和原始记录。时间上限是止损值，不是承诺；扩大范围须在查看正式测试结果前记录，不能无上限重试。
...
| 下一执行包 | P2/P3 教师标定累计限 2 小时；按摘要/持久进度较大值已计约 16.66 分钟，纠偏和重测继续扣同一额度；只用冻结训练家族，不打开留出集 |
...
**P2 协议与限定 P3 标定执行包已完成，结论为停止扩大采样。下一执行包是 T0 纠偏与证据存储优化。** 顺序如下：

1. 复用协议、环境、契约和规则资源；保持 B1 固定及全部家族分母。教师升级另存源码/配置版本，修改目标则另登记协议版本。
2. 在已有训练反例上让明确无响应/一次灰流丽策略在探针内正确结算，所有原生微选择计入预算。补足公共模块提案，而不把 0/20 的精确起手来源覆盖解释为产品能力。
3. 校准可达终场与关键资源的评分，处理实际抽牌后的两次循环，先做少量同条件反事实重放，再重测 20 个训练家族；原失败记录不覆盖。
4. 验证原生 JSONL/会话证据的无损归档与按需恢复，并优化不必要的重放和通信。当前压缩收集器副本不是整个证据目录的存储替代方案。
5. 重新核对 200 家族的保守 4 小时及剩余磁盘预算；未满足则继续停止，或在独立评测前登记较小研究范围。通过后才运行 40 个验证家族选择教师。
6. 教师在验证集有价值且成本可控后，再采样并训练学生；本地 LLM 对照仍按 P3 条件另测，不以文本速度或开发拟合代替策略收益。

本次已交付冻结协议、家族划分、T0 原型、20 家族成本/来源报告与停止结论。后续不自动启动全量 200/2,000 家族采样、付费调用、新框架安装或正式界面接入。
...
- [P2 小试协议](ai-learning-p2-protocol.md)、[P3 标定与成本](ai-learning-p2-p3-results.md)：v4 修订的实测依据，含 20 家族配对、教师失败、压缩恢复及停止扩展结论。
