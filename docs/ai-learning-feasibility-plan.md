...
版本：v6，2026-09-21。依据[资源与时间优化](ai-learning-p3-resource-results.md)和[验证中止与原生死锁修复](ai-learning-p3-validation-results.md)修订。**P2 的 200 家族与划分不变；T0 在 20 个训练家族达标 15/20，B1 13/20，当时版本的 200 家族保守时间降至 3.76 小时。随后验证完成 24/40 对，已完成部分 T0 达标 17 个、B1 14 个，第 25 个家族因原生死锁中止；死锁已修复，两个失败家族的 12 次回归通过，但完整验证未通过。累计 worker 约 87.18 分钟，剩约 32.82 分钟，小于原完整验证保守预测 45.10 分钟，继续停止扩大采样。** P0A 与限定 P0B/P1 保留原结论；整体仍为部分可行。
...
5. 200 家族按 120/40/40 冻结，ZIP/gzip 无损证据与旧记录保留。优化版本的保守新增空间约 4.35 GiB、时间约 3.76 小时，允许进入了验证；随后发现并修复原生焦点锁死锁。新二进制与旧登记分开，尚无同版本完整标定/验证，且剩余累计预算不足原保守完整验证预测，因此不启动全量或正式采样。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | 新标定 40 次、部分验证 48 次和修复后 12 次回归通过完整重放与归档审计；原生死锁失败保留，完整验证中止 | 修复后同版本完整运行与正式数据逐条审计 |
| H2 | 教师数据比现有基线有价值 | 训练标定 T0 15/20、B1 13/20；验证仅完成 24/40 对，其中 T0 17、B1 14，无完整选型结论 | 完整验证的预登记目标、共同支持范围及成本门槛 |
...
当前基础包括[学生训练与执行试验](../experiments/ygo_learning/README.md)、[P0B/P1 结果](ai-learning-p0b-p1-results.md)、[P2 协议](ai-learning-p2-protocol.md)、[P3 标定结果](ai-learning-p2-p3-results.md)、[P3 纠偏结果](ai-learning-p3-correction-results.md)、[资源与时间优化](ai-learning-p3-resource-results.md)、[验证与死锁修复](ai-learning-p3-validation-results.md)、[原模型试验](../experiments/ygo_agent/README.md)及[模块化文档](modular.md)。P0A 的历史证据来自 `2234928`，后续批次保留单独的源码与资源清单。正式运行仍须冻结实际代码、特征、引擎与资产版本。
...
| P3 教师质量 | 训练标定 T0 15/20、B1 13/20，当时预算通过；验证完成 24/40 对后因死锁中止，修复后 12 次回归通过 | 登记修复后的版本与预算，完成同版本标定及全部验证；不能混用部分结果 | 工程修复完成，完整选型未通过 |
...
原 P2–P6 的 6–12 个有效工作日、P7/P8 的 2–4 天只作历史工作量规划，不是计算耗时承诺。旧原生版本的标定预算通过后已进入验证，但完整验证因死锁中止；修复后的版本尚需重新登记与核验，剩余 worker 预算不足原保守完整验证预测，不能按旧排期直接采样。P1、P3 标定、部分验证和修复诊断的成本分别记录，不互相替代。
...
200 家族小试的本节冻结工作已执行，见 [P2 协议](ai-learning-p2-protocol.md)。验证/留出尚未运行，正式 2,000 家族尚未生成。分组由起手与预先定义的结构条件决定，不按本轮教师结果改变。
...
用户约束为零付费调用。已有环境、模型与证据继续保留；下列上限不因阶段完成而放宽。优化版本的训练标定曾给出 200 家族保守 3.76 小时、含恢复预留 4.35 GiB 的通过估计；随后验证发生死锁，修复后二进制改变，这些数值不能替代新版本完整成本验收。新会话原始字节用经恢复核对的 ZIP/gzip 保存，被占用的重复文件保留并计入空间。时间上限是止损值，不是承诺；不能用部分结果或重新启动重置累计预算。
...
| 下一执行包 | P2/P3 worker 累计限 2 小时；已计约 87.18 分钟，余约 32.82 分钟，低于原完整验证保守预测 45.10 分钟；先登记修复后版本及可行预算，留出集继续封存 |
...
**资源取舍与时间优化已完成，随后发现的原生焦点死锁已修复；完整 40 家族验证中止，不能进入 P4。下一执行包先处理修复后原生版本的完整复验与预算登记。**

1. 保留 P2、B1、全部分母、随机条件、原验证登记以及各失败/诊断批次。教师固定 `T0-resource-v7`，不得根据这次部分验证临时修改后拼接结果。
2. 两个失败家族的三轮 B1/T0 回归与确定性焦点锁竞争检查已经通过；下一次运行仍须绑定修复后原生哈希，旧登记不能直接复用。
3. 当前余约 32.82 分钟，原完整验证保守预测约 45.10 分钟，还不包含新原生版本标定。先登记符合剩余额度的方案或明确的新预算，不自动放宽 2 小时上限；若缩小研究问题，保留原失败分母并另登记协议，holdout 继续封存。
4. 在获得可行预算后，先做同版本标定、B1 完整前缀一致性与成本核对，再运行全部 40 个验证家族；48 次旧版部分验证和 12 次修复回归不混成完整结论。
5. 存储继续逐字节恢复审计；已验证但短暂占用的重复文件保留并计入成本。异常控制器轨迹、原生日志、归档、恢复点与环境全部保留，不清空旧实验目录。
6. 完整预登记验证及保守成本同时满足后才进入 P4。正式学生训练、LLM 对照、2,000 家族、能力更新与产品接入继续等待各自门槛。

本次交付包含资源评分修正、测试动画提速、分项计时、重复校验合并、同条件测试导入、完整 20 家族重测、预登记验证入口、24/40 对验证中止证据、原生死锁修复及 12 次复验。未自动启动全量 200/2,000 家族、正式学生训练、付费调用、新框架安装或界面接入。
