...
版本：v5，2026-09-21。依据 [P0B/P1 实测记录](ai-learning-p0b-p1-results.md)、[原 P2/P3 标定](ai-learning-p2-p3-results.md)及[教师纠偏与无损归档](ai-learning-p3-correction-results.md)修订。**P2 的 200 家族与划分不变；新 T0 在 20 个训练家族达标 13/20，原版 10/20，B1 13/20。40 次完整重放与归档恢复通过，存储门槛通过，但 200 家族保守时间约 8.44 小时，仍超过 4 小时。继续停止扩大采样。** P0A 与限定 P0B/P1 保留原结论。独立教师质量、未见起手收益、能力更新及产品接入仍未验证，整体结论为部分可行。
...
4. 已修正声明的对手响应边界，加入公共示范动作提案、资源/合法起动机会评分和循环处理。最终新 T0 的 20 个回合均结束，但只有 13 个达标；不能把消除循环当成策略成功。学生 GPU 训练短测仍不能代表整体提速。
5. 200 家族按 120/40/40 冻结。新会话采用经恢复验证的 ZIP/gzip 归档，旧证据保留；200 家族保守新增空间连同临时恢复预留约 4.99 GiB，低于约 7.94 GiB 剩余额度。保守时间约 8.44 小时仍超限，优化并重测前不启动全量或正式采样。
...
| 编号 | 假设 | 当前状态 | 仍需取得的证据 |
| --- | --- | --- | --- |
| H1 | 自动生成轨迹真实、可复现 | P1 的 115 个会话通过；原/纠偏 P3 各 40 次运行完整重放通过，新批次原生会话无损恢复及日志审计通过 | 正式教师批量数据仍须逐条重放与来源审计 |
| H2 | 教师数据比现有基线有价值 | 新 T0 13/20，原 T0 10/20，B1 13/20；共同支持 19 家族为 T0 12/19、B1 13/19，独立收益未验证 | 修正教师后，在独立局面验证目标达成率或覆盖改善 |
...
当前基础包括[学生训练与执行试验](../experiments/ygo_learning/README.md)、[P0B/P1 结果](ai-learning-p0b-p1-results.md)、[P2 协议](ai-learning-p2-protocol.md)、[P3 标定结果](ai-learning-p2-p3-results.md)、[P3 纠偏结果](ai-learning-p3-correction-results.md)、[原模型试验](../experiments/ygo_agent/README.md)及[模块化文档](modular.md)。P0A 的历史证据来自 `2234928`，后续批次保留单独的源码与资源清单。正式运行仍须冻结实际代码、特征、引擎与资产版本。
...
| P3 教师质量 | T0 纠偏及完整 20 家族重测完成；新教师与 B1 同为 13/20；存储通过，时间预算未通过 | 继续优化成本与资源取舍，保守预算通过后再进入验证集 | 纠偏标定完成，独立质量未评测 |
...
原 P2–P6 的 6–12 个有效工作日、P7/P8 的 2–4 天只作历史工作量规划，不是计算耗时承诺。当前独立教师质量未验证，扩展时间预算未通过；存储已通过本轮检查，不能按旧排期直接采样。P1 的 50 回合成本和 P3 的 20 家族配对成本分别记录，不互相替代。
...
已完成原标定及[纠偏重测](ai-learning-p3-correction-results.md)：新 T0 在全部 20 个训练家族达标 13/20，保留原版 10 个达标家族并新增 3 个；B1 仍为 13/20。新教师 20 个回合均结束，原生重放与完整归档恢复通过，但共同支持范围未超过 B1，保守时间预算仍未通过。搜索可结算预声明的无响应/一次灰流丽条件，公共示范已提供可重新绑定的动作提案；未知随机结果仍在评分前停止。固定配置为 24 节点、12 层原生决策、2 秒检查时限，所有微选择计入预算。以下正式教师要求继续保留，本轮不视为独立质量门槛通过。
...
用户约束为零付费调用。已有环境、模型与证据继续保留；下列上限不因阶段完成而放宽。P3 已完成纠偏后的 20 个训练家族重测；200 家族含恢复临时空间的保守存储估计约 4.99 GiB，低于约 7.94 GiB 剩余额度，但保守时间约 8.44 小时仍超预算。新会话原始字节以经过完整恢复验证的 ZIP/gzip 保存，旧原始证据继续保留。时间上限是止损值，不是承诺；扩大范围须在查看正式测试结果前记录，不能无上限重试。
...
| 下一执行包 | P2/P3 教师标定累计限 2 小时；按摘要/持久进度较大值已计约 49.64 分钟，后续纠偏和重测继续扣同一额度；只用冻结训练家族，不打开留出集 |
...
此指标来自引擎中的连续展开。另报告受干扰子集表现、支持范围、失误位置、终止/回退原因和耗时；训练准确率、原教师轨迹上的单步命中率只作诊断。78/78 的原探针拟合、423 决策的 98.35% 开发拟合、44/50 真实结束回合以及纠偏后训练标定的 13/20 达标，均不满足本节的正式质量门槛。
...
**T0 纠偏与原生证据无损归档执行包已完成；存储通过，独立策略收益与扩展时间尚未通过。下一执行包是教师资源取舍与时间成本优化，继续只用冻结训练家族。**

1. 保持协议、B1、全部家族分母与相同随机条件，保留原版、失败尝试和 `T0-correction-v5` 的独立源码/配置。累计计算已约 49.64 分钟，后续仍扣同一个 2 小时额度。
2. 分别处理仍失败的实际抽牌与保留手牌场景。新 T0 虽由 10/20 提高到 13/20，仍与 B1 总体持平；共同支持 19 家族为 12/19 对 13/19，覆盖改善不能冒充策略增强。
3. 分项测量搜索前缀重建、动态快照、通信、归档解码及重复校验开销；优化后保留至少一次独立重放和一次无损恢复审计，不能通过省略有意义的原生选择或失败证据降低成本。
4. 新会话继续使用已验证的紧凑存储，读取旧批次时兼容原始证据；任何新格式都先验证完整恢复。保留原始字节、报告、恢复点和环境，不清空旧实验目录。
5. 按新的训练标定成本重新核对 200 家族保守 4 小时及磁盘峰值。当前为 8.44 小时，因此仍停止；若需要缩小研究范围，在任何独立评测前另登记协议，不能事后删除失败家族或降低门槛。
6. 只有预算满足后才进入 40 个验证家族选择教师；验证显示价值后才采样并训练学生。本地 LLM 对照、正式 2,000 家族、能力更新与产品接入继续等待各自门槛。

本次交付包含教师修正、公共动作提案、微选择/循环预算、无损原生归档及恢复审计、20 家族完整重测、隐藏信息替换检查与成本/停止报告。未自动启动全量 200/2,000 家族采样、付费调用、新框架安装或正式界面接入。
