...
版本：0.5。记录日期：2026-09-23。状态：讨论中；本文描述的新增能力尚未实现。
...
| C11 | 已确认 | 主 TAG 是构筑中浓度最高的系列部分；两个系列浓度相近时，可以有两个主 TAG。相近的数值阈值尚未确定。 |
...
**浓度的具体实现口径仍待确认**：现有构筑计算使用主卡组与额外卡组的实际副本数；初版推荐参数与重叠处理见 3.9.1，尚未写入程序。系列父子 TAG 或成员重叠可能使两项同时较高，应展示成员依据并单独处理，不把用途 TAG 的数量当作第二个主系列。构筑浓度与方案参与度分别记录。
...
本节是 2026-09-23 根据用户提问给出的推荐方案 D01—D04，没有新增“已确认”结论。数值是可配置的初版产品参数，尚未用真实构筑校准，不是官方卡组分类标准或统计可靠性的证明。本轮仍只维护讨论文档。
...
| Q03 | 第 1 点 | 自动用途和个人标记如何配合，哪些信息允许人工补充？ | 编辑边界已确认；允许字段与保留人工覆盖的推荐答案见 D03，待确认；AI 默认启用方式仍待细化。 |
...
| Q11 | 第 1 点 | 主 TAG 如何确定，是否要先指定示例牌组？ | 主 TAG 定义已确认：浓度最高，接近时双主；暂未指定实例，先继续通用流程讨论。 |
| Q12 | 第 1 点 | 怎样判断相同结果或增强？ | 原则已确认；四种有效状态、共享次数及结果比较的推荐答案见 D04，待确认。 |
| Q13 | 第 1 点 | 构筑浓度采用什么范围与权重，什么差距属于“相近”，重叠系列如何处理？ | 推荐沿用主卡加额外、共享成员均分、80% 相对接近与第二项 15% 最低规模，见 D01；待确认和真实构筑校准。 |
| Q14 | 第 1 点 | 主要效果归纳采用哪些代表方案，参与和重复次数怎样统计？ | 推荐 6—10 组代表方案，覆盖优先、次数辅助，并保留样本门槛与并列显示规则，见 D02；待确认。 |
