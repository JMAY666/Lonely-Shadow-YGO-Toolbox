...
版本：0.3。记录日期：2026-09-23。状态：讨论中；本文描述的新增能力尚未实现。
...
| C07 | 已确认 | 先对主 TAG 内主要卡片的具体效果和系列内部分工进行标记，理解系列组成与运转思路；通用手坑等采用统一效果表示，小轴记录内部作用及与主系列、其他构筑的关系。 |
| C08 | 已确认 | 人与 AI 使用同一套标记系统，使用方式可选；具体模式、默认选项和冲突处理细则继续讨论。 |
...
| 增强补点／延伸 | 在原有一卡动或其他基线上增加场面、伤害、干扰或保留资源；说明额外投入及新增限制。 |
| 试探／骗康 | 一种操作意图：用有价值的动作争取引出对方干扰，再保留其他路线。对方可能不响应，必须同时说明放行后的实际收益。 |
| 扩大场面 | 在已有可接受结果上，投入更多资源增加场面、伤害或续航；比较收益与过度投入的风险。 |

例如，在相同已列明前提下，方案 A 仅需起手卡甲达到目标 X，方案 B 仅需起手卡乙也达到 X，那么两者各有一卡动证据；选择 A 为本局基线时，卡乙可被描述为替代补点。若甲加乙能达到包含 X 并有额外收益的目标 Y，则乙还可能是增强补点。该例只说明评价关系，实际可执行性仍需核对资源和限制。
...
**已确认方向**：以决斗者本局构筑为范围，先认识主 TAG 的组成和运转，再关联通用卡与小轴。主 TAG 表达系列归属，具体效果说明卡能做什么，系列／小轴分工说明为什么在这副牌里使用它。

建议用同一套表示方式组织下列信息，具体字段和存储设计仍待实施前确定：

| 层次 | 记录内容 | 复用与作用范围 |
| --- | --- | --- |
| 通用效果 | 卡牌的具体效果、条件、费用、得到或消耗的资源、限制和来源。 | 同一效果在不同卡组中共用依据；手坑、护航和解场用途使用一致的描述方式。 |
| 主系列分工 | 主 TAG 内主要卡片各效果的作用、需要的前置资源、接到哪些卡／步骤，以及所在展开环节。 | 在系列内说明启动、检索、调度、中间组件、终场和续航的关系；不能只给每张卡贴一个词。 |
...
系列运转说明建议包含“从哪里启动 → 获得什么资源 → 通过哪些中间组件转换 → 达到什么结果 → 保留什么后续”，每条联系能指向卡文或已有方案。AI 可参与整理这些说明，但资料不足的连接需要保留为待核对。
...
**已确认方向**：人和 AI 是同一套标记系统的不同使用者，使用方式可选；它们编辑相同对象、字段、作用范围和关系，接受相同的格式与规则核对。来源用于追溯，不直接决定谁的结论永远优先。

可选模式的初步建议是“人工维护”“AI 提议后采用”“AI 在选定范围自动维护”。这些选项名称、默认模式、自动写入范围和覆盖规则尚未确定，不在本轮启动 AI 标记或实际写入个人资料。

建议区分两种情况：

- **不同上下文的不同分工**：同一卡在不同系列、构筑或方案里承担不同角色，允许并存，不当作冲突。
- **同一上下文的相互矛盾修改**：按所选策略保留当前结果并列出候选、保护指定已确认项目，或允许更新指定范围；默认策略待讨论。保留新旧内容、修改者、依据和版本，避免后台任务覆盖后来的人工作业。

人工和 AI 都不能通过改标签改变卡文事实或将未验证路线写成已验证。资料变化后重新检查受影响的关联；已保存方案仍保留其原始资料和记录快照。具体采用与修正入口复用同一套编辑流程，不分别建设互不相通的人工作业库和 AI 作业库。
...
1. 明确本次评价的构筑版本、方案／分支与起点，并选择目标结果；说明“同样结果”具体比较哪些终场卡、有效效果、保留资源和限制。目标等价规则仍需讨论，不能只比较卡牌数量。
...
4. 以选定路线为基线比较其他方案：相同目标的替代入口标明替代补点；受阻后能继续的分支标明续接补点；增加目标收益的路线标明增强补点，并说明增加了哪些投入。
...
1. **准备构筑知识**：选定主 TAG、通用卡、小轴及标记方式；逐效果记录主系列分工，再关联小轴与主系列的运转关系。资料准备与进入实战后的分析分开。
2. **关联展开方案**：从已有方案取得起手、目标和组件依赖，建立带前提的一卡动、补点及相对废件证据；哪些资料可复用、哪些需要补充分别显示。
3. **读取本局输入**：确认初始手牌、本局构筑和环境。保存原始起手，另行维护当前手牌与已确认消耗；不同用途引用同一个卡片副本。
4. **核对依赖与冲突**：将通用效果、系列分工和方案依据应用到本局，检查费用、持续限制、同名次数、配套卡所在区域，以及干扰与自己路线之间的影响。
5. **形成整手概览**：按本节七项顺序，先给手坑概况和关键冲突，再给护航／解场覆盖、启动与补点、卡手原因及初步取胜方向。不同用途数量可能重叠，不能直接相加成手牌总数或确定阻抗数；每条判断能查看对应方案与条件。
6. **随对局补全判断**：对方牌组未知时给带前提的方向；对方动作、己方交坑或抽牌发生后更新相关结论。若资料缺失则具体指出，不用未知的第六张牌填补启动或斩杀条件。

**界面草案**：保留逐张卡图，旁边按“手坑与冲突 → 护航／解场 → 启动／补点 → 节奏与卡手”展示概览。展开某一结论时能看到涉及的牌、依据、受影响的路线和待核对条件。先做可解释的关系，不预设整手总评分。

**需要继续讨论**：首个主 TAG 与示例牌组是什么？先标记哪些效果和关系？人工／AI 可选模式的默认值及同一上下文冲突策略是什么？方案目标如何判断相同或增强？第一版支持哪些可验证关系与路线？
...
- 人工与 AI 对相同对象写入同一结构的标记，均能在同一编辑器查看、修改并追溯来源；冲突按选定策略处理。
...
| P2：起手分析 | 先准备统一效果标记、主系列／小轴分工和方案角色证据，再按个人顺序分析七项起手内容；人工与 AI 共用维护系统。 | 同一上下文的修改可追溯；不同构筑角色不混用；多用途不重复计资源，冲突、路线依赖和相对废件均可解释。 |
...
| Q03 | 第 1 点 | 自动用途和个人标记如何配合，哪些信息允许人工补充？ | 已确认人与 AI 共用一套可选标记系统；模式默认值、自动写入范围和同一上下文冲突策略待定。 |
...
| Q09 | 第 1 点 | 启动、补点、护航与废件的基础资料如何维护，并关联到具体牌组／路线？ | 方向已确认：主 TAG 内部分工、通用效果、小轴关系共同提供知识；一卡动／补点／废件以展开方案和目标为依据。 |
| Q10 | 第 1 点 | 首版怎样展示手坑作用、卡片冲突和整手倾向，哪些结论需要引擎验证后才提供？ | 待讨论 |
| Q11 | 第 1 点 | 选哪个主 TAG、示例牌组及已有方案，先做一份完整的效果／分工／路线关联样例？ | 待讨论 |
| Q12 | 第 1 点 | “到达相同结果”与“增强场面”比较哪些目标、有效效果、保留资源和限制？ | 待讨论；需要可核对的目标定义。 |
