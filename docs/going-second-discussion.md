...
版本：0.4。记录日期：2026-09-23。状态：讨论中；本文描述的新增能力尚未实现。
...
**浓度的具体实现口径仍待确定**：现有构筑计算使用主卡组与额外卡组的实际副本数；沿用这一口径还是调整权重、差距多小属于“相近”，后续再定，不在初稿中擅自设定百分比。系列父子 TAG 或成员重叠可能使两项同时较高，应展示成员依据并单独处理，不把用途 TAG 的数量当作第二个主系列。构筑浓度与方案参与度分别记录。
...
**需要继续讨论**：浓度“相近”的阈值与成员重叠如何处理？基本展开思路选取哪些代表方案，怎样统计参与与重复使用？通用效果开放哪些受控字段？终场效果的有效性、共享次数和结果一致性怎样具体核对？这些问题不影响本轮已确定的方向，暂不要求指定某个试点系列。
...
| Q03 | 第 1 点 | 自动用途和个人标记如何配合，哪些信息允许人工补充？ | 已确认：人工只选择性修改归类确定的通用表示，局部只读、一般补备注；允许字段、版本冲突细节和 AI 默认启用方式待细化。 |
...
| Q12 | 第 1 点 | 怎样判断相同结果或增强？ | 原则已确认：相同是结果一样，增强主要看终场效果更多；有效性、共享次数和具体结果比较细则待定。 |
| Q13 | 第 1 点 | 构筑浓度采用什么范围与权重，什么差距属于“相近”，重叠系列如何处理？ | 待讨论；现有主卡组加额外按副本数统计可作为起点。 |
| Q14 | 第 1 点 | 主要效果归纳采用哪些代表方案，参与和重复次数怎样统计？ | 待讨论；建议区分跨方案覆盖、实际使用与重复录制。 |
